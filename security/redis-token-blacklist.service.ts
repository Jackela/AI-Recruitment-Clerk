...
export class RedisTokenBlacklistService implements OnModuleDestroy {
  ...
  async isUserBlacklisted(userId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Error checking user blacklist: ${error.message}`);
      return false;
    }
  }
  ...
  private async loadMetricsFromRedis(): Promise<void> {
    try {
      ...
      this.logger.debug(`Could not load metrics from Redis: ${error.message}`);
    }
  }
  ...
  private async updateMetricsInRedis(): Promise<void> {
    try {
      ...
      this.logger.debug(`Could not update metrics in Redis: ${error.message}`);
    }
  }
  ...
  async emergencyClearBlacklist(): Promise<void> {
    try {
      ...
      this.logger.error(`Emergency blacklist clear failed: ${error.message}`);
      throw error;
    }
  }
}