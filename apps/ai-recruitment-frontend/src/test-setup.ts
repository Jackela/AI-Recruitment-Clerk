...
interface JasmineInterface {
  createSpyObj: (name: string, methods: string[], props?: Record<string, unknown>) => Record<string, JasmineSpy>;
  createSpy: (name?: string) => JasmineSpy;
}

(global as { jasmine?: JasmineInterface }).jasmine = {
  createSpyObj: (_name: string, methods: string[], props?: Record<string, unknown>) => {
    ...
    methods.forEach(method => {
      ...
      spyWithMethods.and = {
        ...
        callFake: (fn: (...args: unknown[]) => unknown) => jestSpy.mockImplementation(fn),
        callThrough: () => jestSpy.mockImplementation(),
        stub: () => jestSpy.mockImplementation(() => {}),
        throwError: (error: Error | unknown) => jestSpy.mockImplementation(() => { throw error; })
      };
  ...
  createSpy: (_name?: string) => {
    ...
    spyWithMethods.and = {
      returnValue: (value: unknown) => jestSpy.mockReturnValue(value),
      returnValues: (...values: unknown[]) => jestSpy.mockReturnValueOnce(...values),
      callFake: (fn: (...args: unknown[]) => unknown) => jestSpy.mockImplementation(fn),
      callThrough: () => jestSpy.mockImplementation(),
      stub: () => jestSpy.mockImplementation(() => {}),
      throwError: (error: Error | unknown) => jestSpy.mockImplementation(() => { throw error; })
...
(global as { spyOn?: (object: Record<string, unknown>, method: string) => JasmineSpy }).spyOn = (object: Record<string, unknown>, method: string) => {
