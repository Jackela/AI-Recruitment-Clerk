...
export class ConsentManagementComponent implements OnInit, OnDestroy {
  ...
  readonly processingPurposes: ProcessingPurposeInfo[] = [
    {
      ...
      description:
        'Core functionality required for the recruitment platform to work, including account management and resume processing.',
    ...
    },
    {
      ...
      description:
        'Basic usage statistics to improve platform performance and user experience. No personal identification.',
    ...
    },
    {
      ...
      description:
        'Detailed behavioral analysis to personalize your experience and provide better job recommendations.',
    ...
    },
    {
      ...
      description:
        'Email newsletters, job alerts, and promotional communications about new features.',
      legalBasis: 'Consent (GDPR Article 6(1)(a))',
      dataCategories: [
    ...
    },
    {
      ...
      description:
        'Sharing resume data with external AI services for enhanced analysis and job matching.',
      legalBasis: 'Consent (GDPR Article 6(1)(a))',
      dataCategories: [
        DataCategory.RESUME_CONTENT,
        DataCategory.JOB_PREFERENCES,
  ...
  private initializeForm(): void {
    const purposeControls = this.processingPurposes.map((purpose) =>
      this.fb.group({
        ...
        consentText: [
          `I consent to ${purpose.displayName.toLowerCase()}: ${purpose.description}`,
        ],
      }),
    );
  ...
  private async loadCurrentConsent(): Promise<void> {
    ...
    this.isLoading = true;
    try {
      this.currentConsentStatus = await this.privacyApi.getConsentStatus(
        this.userId,
      );
  ...
  async withdrawConsent(purpose: ConsentPurpose): Promise<void> {
    ...
    try {
      ...
      this.toast.success(
        `Consent for ${purposeInfo.displayName} has been withdrawn`,
      );

      if (this.mode === 'update') {
        const updatedStatus = await this.privacyApi.getConsentStatus(
          this.userId,
        );
        this.consentUpdated.emit(updatedStatus);
      }
