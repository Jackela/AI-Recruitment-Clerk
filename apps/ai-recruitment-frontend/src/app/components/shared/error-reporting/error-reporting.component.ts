      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <form [formGroup]="reportForm" (ngSubmit)="onSubmit()">
            ...
            <div class="form-group">
              ...
              <div class="step-inputs">
                <div *ngFor="let step of reproductionSteps(); let i = index" class="step-input">
                  ...
                  <input
                    ...
                    placeholder="描述操作步骤..."
                  />
                  <button type="button" (click)="removeReproductionStep(i)" class="remove-step">
              ...
              <button type="button" (click)="addReproductionStep()" class="add-step-btn">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
          ...
          <div class="user-guidance" *ngIf="errorSummary()?.userGuidance">
            <h4>建议的解决方案</h4>
            <div class="guidance-content" [innerHTML]="formatGuidance(errorSummary()!.userGuidance)"></div>
          </div>
        </div>
      </div>
    </div>
...
export class ErrorReportingComponent {
  ...
  show(errors?: StructuredError[]): void {
    if (errors && errors.length > 0) {
      this.errors = errors;
      this.errorSummary.set(this.errorReporting.generateErrorReportSummary(errors));
    } else {
      ...
      if (recentErrors.length > 0) {
        ...
        this.errorSummary.set(this.errorReporting.generateErrorReportSummary(recentErrors));
      }
    }
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      const reproductionSteps = this.reproductionSteps().filter(step => step.trim() !== '');

      const result = await this.errorReporting.submitErrorReport(
        this.errors,
        formValue.feedback,
