  ...
  template: `
    <div class="progress-tracker" [class.connected]="isConnected$ | async">
      ...
      <div class="connection-status">
        ...
        <span class="status-text">{{ getStatusText(connectionStatus$ | async) }}</span>
      </div>
      ...
      <div class="overall-progress">
        <div class="progress-header">
          <h3>分析进度</h3>
          <span class="progress-percentage">{{ (overallProgress$ | async) || 0 }}%</span>
        </div>
        ...
        <div class="progress-info">
          <span class="current-step">{{ (currentStep$ | async) || '等待开始...' }}</span>
          ...
          *ngFor="let step of steps; trackBy: trackByStepId"
          ...
          <div class="step-icon">
            <ng-container [ngSwitch]="step.status">
              <svg *ngSwitchCase="'completed'" class="icon-completed" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
              </svg>
              <svg *ngSwitchCase="'error'" class="icon-error" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd" />
              </svg>
              <div *ngSwitchCase="'active'" class="spinner"></div>
              <div *ngSwitchDefault class="step-number">{{ getStepNumber(step.id) }}</div>
            </ng-container>
          </div>
          <div class="step-content">
            <div class="step-label">{{ step.label }}</div>
            <div class="step-description" *ngIf="step.description">{{ step.description }}</div>
            <div class="step-progress" *ngIf="step.status === 'active' && step.progress !== undefined">
      ...
      <div class="message-log" *ngIf="showMessageLog">
        ...
        <div class="log-container">
          <div 
            class="log-entry" 
            *ngFor="let message of recentMessages$ | async; trackBy: trackByMessage"
            [class]="message.type">
...
export class ProgressTrackerComponent implements OnInit, OnDestroy {
  ...
  private connectToWebSocket(): void {
    ...
    // 监听进度更新
    this.webSocketService.onProgress(this.sessionId)
      .pipe(takeUntil(this.destroy$))
      .subscribe(progress => {
    ...
    // 监听完成事件
    this.webSocketService.onCompletion(this.sessionId)
      .pipe(takeUntil(this.destroy$))
    ...
    // 监听错误事件
    this.webSocketService.onError(this.sessionId)
      .pipe(takeUntil(this.destroy$))
      .subscribe(error => {
  ...
  private handleCompletion(completion: any): void {
  ...
  private updateStepProgress(currentStep: string, progress: number): void {
    const step = this.steps.find(s => s.label.includes(currentStep) || s.id === currentStep);
    if (step) {
      step.status = 'active';
