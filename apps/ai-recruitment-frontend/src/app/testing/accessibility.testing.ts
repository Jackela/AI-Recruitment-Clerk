...
export class AccessibilityTester {
   ...
   */
  static testComponent(fixture: ComponentFixture<any>): AccessibilityTestResult {
   ...
   */
  private static testAriaAttributes(element: HTMLElement): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
    ...
    interactiveElements.forEach((el: Element) => {
      ...
      const hasLabel = 
        ...
        (htmlEl.tagName === 'INPUT' && htmlEl.getAttribute('type') === 'submit' && htmlEl.hasAttribute('value'));
    ...
    const elementsWithAria = element.querySelectorAll('[aria-live], [aria-expanded], [aria-checked], [aria-selected]');
   ...
   */
  private static testKeyboardNavigation(element: HTMLElement): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
    
    // Test for focusable elements without proper tabindex
    const clickableElements = element.querySelectorAll('[onclick], .clickable, [role="button"]:not(button)');
    clickableElements.forEach((el: Element) => {
      const htmlEl = el as HTMLElement;
      if (htmlEl.tagName !== 'BUTTON' && htmlEl.tagName !== 'A' && !htmlEl.hasAttribute('tabindex')) {
        issues.push({
          type: 'warning',
    ...
    // Test for skip links
    const skipLinks = element.querySelectorAll('.skip-link, [href="#main-content"], [href="#main"]');
   ...
   */
  private static testHeadingStructure(element: HTMLElement): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
   ...
   */
  private static testFocusManagement(element: HTMLElement): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
  ...
  static testWCAG2_1_AA(element: HTMLElement): {
    ...
    const issues = this.testComponent({ nativeElement: element } as ComponentFixture<any>).issues;
    const errors = issues.filter(i => i.type === 'error');
    ...
    const principle2 = !errors.some(e => 
      ['KEYBOARD_NAVIGATION', 'SKIP_NAVIGATION', 'POSITIVE_TABINDEX'].includes(e.rule)
    );
    ...
    const principle4 = !errors.some(e => 
      ['ARIA_LABEL_MISSING', 'ARIA_INVALID_VALUE', 'ARIA_INVALID_BOOLEAN'].includes(e.rule)
    );
