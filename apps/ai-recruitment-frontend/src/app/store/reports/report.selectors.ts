...
export const selectReportsStatistics = createSelector(
  ...
  (reports: ReportListItem[]) => {
    ...
    // Calculate average score across all reports, treating missing scores as 0
    const averageScore =
      reports.length > 0
...
export const selectJobReportsStatistics = (jobId: string) =>
  createSelector(selectReportsByJobId(jobId), (reports: ReportListItem[]) => {
    ...
    const highScoreReports = reports.filter(
      (report) => (report.matchScore || 0) >= 80,
    ).length;
    ...
    return {
      ...
      completionRate: total > 0 ? Math.round((completed / total) * 100) : 0,
      highScoreRate:
        total > 0 ? Math.round((highScoreReports / total) * 100) : 0,
    };
  });
