...
describe('Report Selectors', () => {
  ...
  describe('Basic State Selectors', () => {
    it('should select all reports', () => {
      const result = ReportSelectors.selectAllReports.projector(mockReportState);
      expect(result).toEqual(mockReportListItems);
      expect(result).toHaveLength(4);
    });

    it('should select selected report', () => {
      const result = ReportSelectors.selectSelectedReport.projector(mockReportState);
    ...
    it('should select reports loading state', () => {
      ...
      const result = ReportSelectors.selectReportsLoading.projector(loadingState);
    ...
    it('should select current job ID', () => {
      const result = ReportSelectors.selectCurrentJobId.projector(mockReportState);
    ...
    it('should handle null selected report', () => {
      ...
      const result = ReportSelectors.selectSelectedReport.projector(stateWithoutSelected);
      expect(result).toBeNull();
    });

    it('should handle empty reports array', () => {
      const stateWithoutReports = { ...mockReportState, reports: [] };
      const result = ReportSelectors.selectAllReports.projector(stateWithoutReports);
    ...
    it('should handle null current job ID', () => {
      ...
      const result = ReportSelectors.selectCurrentJobId.projector(stateWithoutJobId);
      expect(result).toBeNull();
    });
  });
  ...
  describe('Derived Selectors', () => {
    it('should select reports count', () => {
      const result = ReportSelectors.selectReportsCount.projector(mockReportListItems);
    ...
    it('should return empty array for non-existent job ID', () => {
      ...
      const selectorFunction = ReportSelectors.selectReportsByJobId('nonexistent');
      const result = selectorFunction.projector(jobReports);
      expect(result).toEqual([]);
    });
    ...
    it('should select reports by status', () => {
      const completedReportsSelector = ReportSelectors.selectReportsByStatus('completed');
      const result = completedReportsSelector.projector(mockReportListItems);
      expect(result).toHaveLength(2);
      expect(result.every(report => report.status === 'completed')).toBe(true);
      expect(result.map(report => report.candidateName)).toEqual(['John Doe', 'Bob Johnson']);
    });

    it('should return empty array for non-existent status', () => {
      const nonExistentStatusSelector = ReportSelectors.selectReportsByStatus('archived');
    ...
    it('should select current job reports', () => {
      ...
      const result = ReportSelectors.selectCurrentJobReports.projector(reportsWithJobIds, 'job1');
      expect(result).toHaveLength(3);
      expect(result.every(report => report.jobId === 'job1')).toBe(true);
    });

    it('should return empty array when no current job ID', () => {
      const result = ReportSelectors.selectCurrentJobReports.projector(mockReportListItems, null);
      expect(result).toEqual([]);
    });
  });
  ...
  describe('UI State Selectors', () => {
    it('should select reports loading state', () => {
      const loading = true;
      const result = ReportSelectors.selectReportsLoadingState.projector(loading);
    ...
    it('should select reports with error state', () => {
      ...
      const loading = false;
      const result = ReportSelectors.selectReportsWithError.projector(error, loading);
  ...
  describe('Complex Derived Selectors', () => {
    it('should calculate reports statistics correctly', () => {
      ...
      const result = ReportSelectors.selectReportsStatistics.projector(reportsWithScores);
    ...
    it('should calculate 100% completion when all reports are completed', () => {
      ...
      const result = ReportSelectors.selectReportsStatistics.projector(allCompletedReports);
    ...
    it('should handle reports with missing scores', () => {
      ...
      const result = ReportSelectors.selectReportsStatistics.projector(reportsWithMissingScores);
  ...
  describe('Job-Specific Report Statistics', () => {
    it('should calculate job reports statistics correctly', () => {
      ...
      const selectorFunction = ReportSelectors.selectJobReportsStatistics('job1');
      const result = selectorFunction.projector(jobReports);
  ...
  describe('Report Management State Selector', () => {
    it('should select report management state with computed properties', () => {
      const result = ReportSelectors.selectReportManagementState.projector(mockReportState);
    ...
    it('should show canCreateReport false when loading', () => {
      const loadingState = { ...mockReportState, loading: true };
      const result = ReportSelectors.selectReportManagementState.projector(loadingState);
    ...
    it('should show hasReports false when no reports exist', () => {
      ...
      const result = ReportSelectors.selectReportManagementState.projector(emptyState);
    ...
    it('should handle all state properties correctly', () => {
      ...
      const result = ReportSelectors.selectReportManagementState.projector(complexState);
  ...
  describe('Selector Memoization', () => {
    it('should return same reference for identical inputs', () => {
      const result1 = ReportSelectors.selectAllReports.projector(mockReportState);
      const result2 = ReportSelectors.selectAllReports.projector(mockReportState);
      
      expect(result1).toBe(result2);
    });

    it('should return new reference for different inputs', () => {
      const modifiedState = { ...mockReportState, reports: [...mockReportListItems, {
        id: 'report5',
    ...
    it('should memoize complex calculations', () => {
      const spy = jest.fn().mockImplementation(ReportSelectors.selectReportsStatistics.projector);
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle state with undefined reports gracefully', () => {
      ...
      expect(() => {
        ReportSelectors.selectAllReports.projector(invalidState);
      }).toThrow();
    });
    ...
    it('should handle reports with missing required fields', () => {
      ...
      const result = ReportSelectors.selectReportsCount.projector(invalidReports);
    ...
    it('should handle filter operations on malformed data', () => {
      ...
      const completedSelector = ReportSelectors.selectReportsByStatus('completed');
    ...
    it('should handle division by zero in statistics calculations', () => {
      ...
      const result = ReportSelectors.selectReportsStatistics.projector(emptyReports);
  ...
  describe('Performance Considerations', () => {
    it('should handle large datasets efficiently', () => {
      const largeReportsList: ReportListItem[] = Array.from({ length: 1000 }, (_, i) => ({
        id: `report${i}`,
      ...
      const result = ReportSelectors.selectReportsStatistics.projector(largeReportsList);
    ...
    it('should efficiently sort recent reports', () => {
      const largeReportsList: ReportListItem[] = Array.from({ length: 1000 }, (_, i) => ({
        ...
        oneSentenceSummary: `Summary for candidate ${i}`,
        status: 'completed',
        generatedAt: new Date(Date.now() - Math.random() * 365 * 24 * 60 * 60 * 1000)
      }));
