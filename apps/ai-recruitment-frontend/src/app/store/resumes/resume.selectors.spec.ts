...
describe('Resume Selectors', () => {
    ...
    workExperience: [
      {
        ...
        endDate: '2024-01-01',
        summary: 'Developed modern web applications using Angular and TypeScript'
      }
    ],
    education: [
      {
  ...
  describe('Basic State Selectors', () => {
    it('should select all resumes', () => {
      const result = ResumeSelectors.selectAllResumes.projector(mockResumeState);
      expect(result).toEqual(mockResumeListItems);
      expect(result).toHaveLength(4);
    });

    it('should select selected resume', () => {
      const result = ResumeSelectors.selectSelectedResume.projector(mockResumeState);
    ...
    it('should select resumes loading state', () => {
      ...
      const result = ResumeSelectors.selectResumesLoading.projector(loadingState);
    ...
    it('should select resume uploading state', () => {
      ...
      const result = ResumeSelectors.selectResumeUploading.projector(uploadingState);
      expect(result).toBe(true);
    });

    it('should select resume upload progress', () => {
      const progressState = { ...mockResumeState, uploadProgress: 75 };
      const result = ResumeSelectors.selectResumeUploadProgress.projector(progressState);
      expect(result).toBe(75);
    });

    it('should handle null selected resume', () => {
      const stateWithoutSelected = { ...mockResumeState, selectedResume: null };
      const result = ResumeSelectors.selectSelectedResume.projector(stateWithoutSelected);
      expect(result).toBeNull();
    });

    it('should handle empty resumes array', () => {
      const stateWithoutResumes = { ...mockResumeState, resumes: [] };
      const result = ResumeSelectors.selectAllResumes.projector(stateWithoutResumes);
  ...
  describe('Derived Selectors', () => {
    it('should select resumes count', () => {
      const result = ResumeSelectors.selectResumesCount.projector(mockResumeListItems);
    ...
    it('should select resumes by status', () => {
      const processedResumesSelector = ResumeSelectors.selectResumesByStatus('processed');
      const result = processedResumesSelector.projector(mockResumeListItems);
      expect(result).toHaveLength(2);
      expect(result.every(resume => resume.status === 'processed')).toBe(true);
      expect(result.map(resume => resume.candidateName)).toEqual(['John Doe', 'Bob Johnson']);
    });

    it('should return empty array for non-existent status', () => {
      const nonExistentStatusSelector = ResumeSelectors.selectResumesByStatus('archived');
      const result = nonExistentStatusSelector.projector(mockResumeListItems);
      expect(result).toEqual([]);
    });

    it('should select processed resumes', () => {
      const result = ResumeSelectors.selectProcessedResumes.projector(mockResumeListItems);
      expect(result).toHaveLength(2);
      expect(result.every(resume => resume.status === 'processed')).toBe(true);
    });
    ...
    it('should select pending resumes', () => {
      ...
      const result = ResumeSelectors.selectPendingResumes.projector(resumesWithPending);
      expect(result).toHaveLength(2);
      expect(result.every(resume => resume.status === 'pending' || resume.status === 'processing')).toBe(true);
    });
  });
  ...
  describe('High-Scoring Resumes Selector', () => {
    it('should select high-scoring resumes with default threshold', () => {
      ...
      expect(result.every(resume => resume.analysis && resume.analysis.overallScore >= 75)).toBe(true);
    });
  ...
  describe('UI State Selectors', () => {
    it('should select resume loading state with combined flags', () => {
      ...
      const result = ResumeSelectors.selectResumeLoadingState.projector(loading, uploading);
    ...
    it('should select resume upload state', () => {
      ...
      const progress = 75;
      const error = null;
      const result = ResumeSelectors.selectResumeUploadState.projector(uploading, progress, error);
    ...
    it('should select resumes with error state', () => {
      ...
      const loading = false;
      const result = ResumeSelectors.selectResumesWithError.projector(error, loading);
  ...
  describe('Complex Derived Selectors', () => {
    it('should calculate resume statistics correctly', () => {
      const result = ResumeSelectors.selectResumeStatistics.projector(mockResumeListItems);
    ...
    it('should handle resumes without analysis in statistics', () => {
      const resumesWithoutAnalysis = [
        ...
        { ...mockResumeListItems[2], status: 'processed' as const, analysis: undefined }
      ];
      
      const result = ResumeSelectors.selectResumeStatistics.projector(resumesWithoutAnalysis);
  ...
  describe('Score Range Selector', () => {
    it('should select resumes by score range', () => {
      const scoreRangeSelector = ResumeSelectors.selectResumesByScoreRange(80, 90);
      const result = scoreRangeSelector.projector(mockResumeListItems);
      
      expect(result).toHaveLength(2); // 85 and 88 are in range 80-90
      expect(result.map(resume => resume.candidateName)).toEqual(['John Doe', 'Alice Williams']);
    });

    it('should handle inclusive score boundaries', () => {
      const scoreRangeSelector = ResumeSelectors.selectResumesByScoreRange(78, 85);
      const result = scoreRangeSelector.projector(mockResumeListItems);
      
      expect(result).toHaveLength(2); // 78 and 85 (inclusive)
      expect(result.every(resume => {
    ...
    it('should return empty array when no resumes in range', () => {
      const scoreRangeSelector = ResumeSelectors.selectResumesByScoreRange(95, 100);
      const result = scoreRangeSelector.projector(mockResumeListItems);
    ...
    it('should handle resumes without scores', () => {
      ...
      const scoreRangeSelector = ResumeSelectors.selectResumesByScoreRange(80, 95);
  ...
  describe('Skill Search Selector', () => {
    it('should select resumes by skill query', () => {
      const skillSearchSelector = ResumeSelectors.selectResumesBySkill('JavaScript');
    ...
    it('should handle case-insensitive search', () => {
      const skillSearchSelector = ResumeSelectors.selectResumesBySkill('typescript');
      const result = skillSearchSelector.projector(mockResumeListItems);
    ...
    it('should handle partial skill matching', () => {
      const skillSearchSelector = ResumeSelectors.selectResumesBySkill('Script');
      const result = skillSearchSelector.projector(mockResumeListItems);
  ...
  describe('Resume Management State Selector', () => {
    it('should select resume management state with computed properties', () => {
      const result = ResumeSelectors.selectResumeManagementState.projector(mockResumeState);
    ...
    it('should show canUpload false when uploading', () => {
      const uploadingState = { ...mockResumeState, uploading: true };
      const result = ResumeSelectors.selectResumeManagementState.projector(uploadingState);
    ...
    it('should show canUpload false when loading', () => {
      const loadingState = { ...mockResumeState, loading: true };
      const result = ResumeSelectors.selectResumeManagementState.projector(loadingState);
    ...
    it('should show hasResumes false when no resumes exist', () => {
      ...
      const result = ResumeSelectors.selectResumeManagementState.projector(emptyState);
    ...
    it('should handle upload progress state', () => {
      ...
      const result = ResumeSelectors.selectResumeManagementState.projector(progressState);
  ...
  describe('Selector Memoization', () => {
    it('should return same reference for identical inputs', () => {
      const result1 = ResumeSelectors.selectAllResumes.projector(mockResumeState);
      const result2 = ResumeSelectors.selectAllResumes.projector(mockResumeState);
      
      expect(result1).toBe(result2);
    });

    it('should return new reference for different inputs', () => {
      const modifiedState = { ...mockResumeState, resumes: [...mockResumeListItems, {
        id: 'resume5',
        jobId: 'job1',
    ...
    it('should memoize complex calculations', () => {
      const spy = jest.fn().mockImplementation(ResumeSelectors.selectResumeStatistics.projector);
  ...
  describe('Edge Cases and Error Handling', () => {
    it('should handle state with undefined resumes gracefully', () => {
      ...
      expect(() => {
        ResumeSelectors.selectAllResumes.projector(invalidState);
      }).toThrow();
    });
    ...
    it('should handle resumes with missing required fields', () => {
      ...
      const result = ResumeSelectors.selectResumesCount.projector(invalidResumes);
    ...
    it('should handle filter operations on malformed data', () => {
      ...
      const processedSelector = ResumeSelectors.selectResumesByStatus('processed');
    ...
    it('should handle division by zero in statistics calculations', () => {
      ...
      const result = ResumeSelectors.selectResumeStatistics.projector(emptyResumes);
    ...
    it('should handle skill search with malformed analysis data', () => {
      ...
      const result = skillSearchSelector.projector(resumesWithMalformedAnalysis);
  ...
  describe('Performance Considerations', () => {
    it('should handle large datasets efficiently', () => {
      const largeResumesList: ExtendedResumeListItem[] = Array.from({ length: 1000 }, (_, i) => ({
        id: `resume${i}`,
        jobId: 'job1',
      ...
      const result = ResumeSelectors.selectResumeStatistics.projector(largeResumesList);
    ...
    it('should efficiently filter resumes by status', () => {
      const largeResumesList: ExtendedResumeListItem[] = Array.from({ length: 1000 }, (_, i) => ({
        ...
        jobId: 'job1',
        originalFilename: `resume${i}.pdf`,
        status: i % 3 === 0 ? 'processed' : (i % 3 === 1 ? 'processing' : 'failed'),
        candidateName: `Candidate ${i}`,
        createdAt: new Date(),
        uploadedAt: new Date()
      }));
    ...
    it('should efficiently search by skills', () => {
      const largeResumesList: ExtendedResumeListItem[] = Array.from({ length: 1000 }, (_, i) => ({
        ...
        analysis: {
          overallScore: 80,
          skills: i % 10 === 0 ? ['JavaScript', 'React'] : ['Python', 'Django']
        }
      }));
      ...
      expect(result.every(resume => 
        resume.analysis?.skills?.some(skill => skill.toLowerCase().includes('javascript'))
      )).toBe(true);
      expect(end - start).toBeLessThan(100); // Should complete reasonably fast
    });
  });
