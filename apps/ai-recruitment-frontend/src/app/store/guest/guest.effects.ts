    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.guestApiService.getUsageStatus().pipe(
          map(usageStatus => GuestActions.loadUsageStatusSuccess({ usageStatus })),
          catchError(error => of(GuestActions.loadUsageStatusFailure({ 
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.guestApiService.getGuestDetails().pipe(
          map(guestDetails => GuestActions.loadGuestDetailsSuccess({ guestDetails })),
          catchError(error => of(GuestActions.loadGuestDetailsFailure({ 
        ...
        this.guestApiService.checkUsage().pipe(
          ...
          catchError(error => {
            if (error.status === 429) {
              ...
              return of(GuestActions.setLimited({ 
                isLimited: true, 
                message: error.error?.message || 'Usage limit exceeded' 
              }));
            }
            return of(GuestActions.incrementUsageFailure({ 
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.guestApiService.generateFeedbackCode().pipe(
          ...
          catchError(error => of(GuestActions.generateFeedbackCodeFailure({ 
            error: error.error?.message || error.message || 'Failed to generate feedback code' 
          })))
        )
      )
    )
      ...
      switchMap(({ feedbackCode }) =>
        this.guestApiService.redeemFeedbackCode(feedbackCode).pipe(
          map(response => {
            ...
            return GuestActions.redeemFeedbackCodeSuccess({ message: response.message });
          }),
          catchError(error => of(GuestActions.redeemFeedbackCodeFailure({ 
            error: error.error?.message || error.message || 'Failed to redeem feedback code' 
          })))
        )
      )
    )
  ...
  uploadResume$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ file, candidateName, candidateEmail, notes }) =>
        this.guestApiService.analyzeResume(file, candidateName, candidateEmail, notes).pipe(
          map(analysisData => {
            // Start polling for results after successful upload
            if (analysisData.data.analysisId) {
              setTimeout(() => {
                this.store.dispatch(GuestActions.loadAnalysisResults({ 
          ...
          catchError(error => {
            if (error.status === 429) {
              ...
              this.store.dispatch(GuestActions.setLimited({ 
                isLimited: true, 
                message: error.error?.message || 'Usage limit exceeded' 
              }));
            }
            return of(GuestActions.uploadResumeFailure({ 
              error: error.error?.message || error.message || 'Failed to upload resume' 
            }));
          })
        )
      )
    ...
    this.actions$.pipe(
      ...
      switchMap(() =>
        this.guestApiService.getDemoAnalysis().pipe(
          map(demoResults => GuestActions.loadDemoAnalysisSuccess({ demoResults })),
          catchError(error => {
            if (error.status === 429) {
            ...
            return of(GuestActions.loadDemoAnalysisFailure({ 
              error: error.error?.message || error.message || 'Failed to load demo analysis' 
            }));
          })
        )
      )
