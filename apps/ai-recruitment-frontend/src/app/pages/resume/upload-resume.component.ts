...
@Component({
  ...
  <div class="container">
    ...
    <!-- 上传表单 -->
    <form (submit)="onSubmit($event)" *ngIf="!analysisId()" class="upload-form">
      <div class="form-section">
        <h3>候选人信息 (可选)</h3>
        <div class="row">
          <label>
            姓名
            <input [(ngModel)]="candidateName" name="candidateName" placeholder="请输入候选人姓名" />
          </label>
          <label>
            邮箱
            <input [(ngModel)]="candidateEmail" name="candidateEmail" type="email" placeholder="请输入邮箱地址" />
          </label>
        </div>
        <label>
          备注
          <textarea [(ngModel)]="notes" name="notes" rows="3" placeholder="添加任何相关备注..."></textarea>
        </label>
      </div>
      ...
      <div class="form-section">
        ...
        <div class="file-upload">
          ...
          <label for="resume-file" class="file-label">
            ...
            <span>{{ file ? file.name : '选择简历文件 (PDF, DOC, DOCX)' }}</span>
          </label>
        </div>
      </div>
      ...
      <div class="actions">
        ...
        <button type="button" (click)="getDemo()" class="demo-btn">体验演示</button>
      </div>
    </form>
    ...
    <div class="results-section" *ngIf="analysisComplete()">
      ...
      <div class="result-actions">
        ...
        <button (click)="downloadReport()" class="download-btn" *ngIf="reportUrl()">
          下载报告
        </button>
      </div>
    </div>
    ...
    this.guestApi.analyzeResume(this.file, this.candidateName, this.candidateEmail, this.notes)
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          this.errorMessage.set(err.error?.message || err.message || '上传失败');
