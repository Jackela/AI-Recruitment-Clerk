    ...
    <div class="error-card" [@slideIn]>
      ...
      <div class="error-content">
        <div class="error-icon" [class]="getIconClass()">
          <svg *ngIf="!isNetworkError()" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <h3 class="error-title">{{ errorInfo?.message || '处理过程中遇到问题' }}</h3>
        ...
        <div class="error-details" *ngIf="showDetails">
          ...
          <div class="error-timestamp" *ngIf="errorInfo?.timestamp">
            ...
            <span class="value">{{ errorInfo?.timestamp ? formatTimestamp(errorInfo?.timestamp) : 'N/A' }}</span>
          </div>
        ...
        <div class="error-suggestions">
          ...
          <ul class="suggestion-list" role="list">
            <li *ngFor="let suggestion of getSuggestions()" role="listitem">{{ suggestion }}</li>
          </ul>
        </div>
        ...
        <div class="troubleshooting-tips" *ngIf="showTroubleshooting">
          <details class="tips-details">
            ...
            <div class="tips-content">
              <div class="tip-item" *ngFor="let tip of getTroubleshootingTips()">
      ...
      <div class="error-actions">
        ...
        <button (click)="onAction('start-new')" 
                class="secondary-btn"
                [disabled]="isRetrying"
                aria-label="重新开始新的分析">
          <svg class="btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
        ...
        <button (click)="onAction('contact-support')" 
                class="outline-btn"
...
export class AnalysisErrorComponent {
  ...
  getTroubleshootingTips(): Array<{title: string, description: string}> {
    return [
      {
        title: '检查文件格式',
        description: '确保上传的是 PDF、DOC 或 DOCX 格式的文件，其他格式暂不支持。'
      },
      {
        title: '文件大小限制',
      ...
      {
        title: '浏览器兼容性',
        description: '建议使用最新版本的 Chrome、Firefox、Safari 或 Edge 浏览器。'
      }
    ];
  }
