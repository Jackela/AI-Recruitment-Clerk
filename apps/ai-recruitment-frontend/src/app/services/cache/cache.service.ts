...
export class CacheService {
  ...
  private async applyEvictionStrategy(newItemSize: number, config: CacheConfig): Promise<void> {
  ...
  private async evictLRU(targetSize: number, config: CacheConfig): Promise<void> {
  ...
  private async evictLFU(targetSize: number, config: CacheConfig): Promise<void> {
    const entries = Array.from(this.memoryCache.entries())
  ...
  private async evictFIFO(targetSize: number, config: CacheConfig): Promise<void> {
    const entries = Array.from(this.memoryCache.entries());
  ...
  preload<T>(keys: string[], factory: (key: string) => Observable<T>, config?: CacheConfig): void {
    keys.forEach(key => {
