...
export class TouchGestureService {
  ...
  private setupGestureHandlers(
    ...
    // Handle start events
    startEvents$.pipe(takeUntil(destroy$)).subscribe(({ event, type }) => {
    ...
    // Handle move events
    moveEvents$.pipe(takeUntil(destroy$)).subscribe(({ event, type }) => {
    ...
    // Handle end events
    endEvents$.pipe(takeUntil(destroy$)).subscribe(({ event, type }) => {
      if (touchState.touches.size === 0) return;
