...
export class PortManager {
   ...
   */
  private async getProcessOnPort(port: number): Promise<{ pid: string; name: string }> {
  ...
  async allocatePort(serviceName: string): Promise<number> {
    ...
    for (const port of portsToTry) {
      ...
      if (status.isAvailable) {
        ...
        console.log(`⚠️ Port ${port} occupied by ${status.processName || 'unknown'} (PID: ${status.processId})`);
      }
    }

    throw new Error(`No available ports found for ${config.name}. Tried: ${portsToTry.join(', ')}`);
  }
  ...
  async forceKillPort(port: number): Promise<boolean> {
    ...
    try {
          ...
          try {
            ...
            for (const line of lines) {
              ...
              if (pid && pid !== '0' && !isNaN(parseInt(pid))) {
                try {
                  await execAsync(`taskkill /PID ${pid} /F /T`, { timeout: 10000 });
                  console.log(`🔪 Force killed Windows process ${pid} on port ${port} (attempt ${attempt + 1})`);
                  killedAny = true;
                } catch (killError) {
                  console.warn(`⚠️ Failed to kill Windows process ${pid} on attempt ${attempt + 1}:`, killError);
                }
              }
            }
            ...
            if (attempt === 2) {
              console.log(`ℹ️ No process found on port ${port} after ${attempt + 1} attempts`);
            }
          }
            ...
            for (const pid of pids) {
              try {
                ...
                try {
                  ...
                  console.log(`🔪 Force killed Unix process ${pid} on port ${port} (attempt ${attempt + 1})`);
                } catch {
                  console.log(`✅ Process ${pid} terminated gracefully on port ${port}`);
                }
                killedAny = true;
              } catch (killError) {
                console.warn(`⚠️ Failed to kill Unix process ${pid} on attempt ${attempt + 1}:`, killError);
              }
            }
      ...
      if (killedAny) {
        ...
        if (!released) {
          console.warn(`⚠️ Port ${port} not released within timeout after killing processes`);
        }
      }
   ...
   */
  private async waitForPortRelease(port: number, timeoutMs: number = 10000): Promise<boolean> {
  ...
  async cleanupAllPorts(): Promise<void> {
    ...
    for (const port of Array.from(allPorts)) {
      try {
        const status = await this.getPortStatus(port);
        if (!status.isAvailable) {
          console.log(`🔄 Cleaning up port ${port} (${status.processName}, PID: ${status.processId})`);
          ...
          if (!verifyStatus.isAvailable) {
            console.warn(`⚠️ Port ${port} still occupied after cleanup attempt`);
          }
        }
      } catch (error) {
  ...
  async waitForService(serviceName: string, timeoutMs: number = 30000): Promise<boolean> {
    ...
    if (!config || !port) {
      throw new Error(`Service ${serviceName} not configured or port not allocated`);
    }
    ...
    while (Date.now() - startTime < timeoutMs) {
      try {
        attempts++;
        console.log(`⏳ Waiting for ${config.name} at ${url} (attempt ${attempts})`);
    ...
    console.error(`❌ ${config.name} failed to start within ${timeoutMs}ms (${attempts} attempts)`);
    return false;
  }
  ...
  async generatePortReport(): Promise<string> {
    ...
    for (const [serviceName, config] of this.serverConfigs) {
      ...
      for (const port of portsToCheck) {
        ...
        const processInfo = status.processName ? ` (${status.processName}, PID: ${status.processId})` : '';
