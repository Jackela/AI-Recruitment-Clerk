...
async function globalSetup() {
  ...
  const devServerUrl = process.env.PLAYWRIGHT_BASE_URL || 'http://localhost:4202';
  ...
  if (!isServerReady) {
    console.error(`❌ Dev server at ${devServerUrl} is not responding after 60 seconds`);
    console.error('   This may cause Firefox connection issues during parallel test execution');
  } else {
