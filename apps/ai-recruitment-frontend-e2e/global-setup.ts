...
import { validateTestEnvironment, logTestEnvironment } from './test-environment';
...
async function globalSetup() {
  ...
  if (!useRealAPI) {
    ...
    try {
      ...
      console.log(`✅ Mock API Server started on dynamically allocated port ${mockServerPort}`);
      ...
      console.log(`⏳ Waiting for mock server at ${mockServerUrl} to be ready...`);
      ...
      if (!isMockReady) {
        ...
        try {
          ...
          const retryReady = await portManager.waitForService('mock-api', 15000);
          if (!retryReady) {
            throw new Error('Mock server failed to start after recovery attempt');
          }
    ...
    try {
      ...
      const fallbackUrl = process.env.GATEWAY_HEALTH_URL || 'http://localhost:3000/api/auth/health';
  ...
  try {
    ...
    console.warn('⚠️ Could not allocate dev server port, using default:', error);
    ...
    if (!isDefaultAvailable) {
      console.warn(`⚠️ Default dev server port ${defaultPort} is occupied, attempting cleanup...`);
  ...
  const devServerUrl = process.env.PLAYWRIGHT_BASE_URL || `http://localhost:${devServerPort}`;
  ...
  try {
    ...
    if (!allHealthy) {
      console.warn('⚠️ Some services are not healthy, but continuing with test setup');
    }
  } catch (error) {
