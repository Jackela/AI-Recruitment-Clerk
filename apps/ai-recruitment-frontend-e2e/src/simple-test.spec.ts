...
test.describe('Simple Angular App Test', () => {
  test('check if arc-root exists and app loads with longer timeout', async ({ page }) => {
    ...
    if (arcRootCount > 0) {
      ...
      console.log('arc-root content preview:', arcRootContent.substring(0, 500));
      ...
      if (hasAppTitle) {
        ...
        const hasJobsContent = await page.locator('nav a').filter({ hasText: '岗位管理' }).count() > 0;
  ...
  test('direct navigation to specific routes', async ({ page }) => {
    ...
    console.log('Create page HTML preview:', createPageContent.substring(0, 1000));
