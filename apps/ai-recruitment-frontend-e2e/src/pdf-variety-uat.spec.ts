  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('request', (request) => {
      if (request.url().includes('/coach/analyze')) {
        console.log(`📤 Analysis Request: ${request.method()} ${request.url()}`);
      }
    });

    page.on('response', (response) => {
      if (response.url().includes('/coach/analyze')) {
        console.log(`📥 Analysis Response: ${response.status()} ${response.url()}`);
      }
    });
  });

  test('Multi-page PDF Test: Skills from last page are correctly identified', async ({ page }) => {
    ...
    expect(fs.existsSync(multiPagePdfPath), `Multi-page PDF not found: ${multiPagePdfPath}`).toBeTruthy();
    ...
    await expect(page.locator('h2').filter({ hasText: /coach.*analysis|coach.*gap/i })).toBeVisible({ timeout: 10000 });
    ...
    const analyzeButton = page.locator('button').filter({ hasText: /analyze|分析|提交|submit/i }).first();
    await expect(analyzeButton).toBeEnabled({ timeout: retryTimeout });

    // Wait for API response with retry logic
    const responsePromise = page.waitForResponse(
      response => response.url().includes('/gap-analysis-file') && response.status() === 200,
      { timeout: baseTimeout }
    );
    ...
    await expect(page.locator('arc-gap-analysis-report')).toBeVisible({ timeout: baseTimeout });
    
    // Wait for the specific h3 "Diagnostic Report" heading to appear
    await expect(page.locator('h3').filter({ hasText: /diagnostic report/i })).toBeVisible({ timeout: baseTimeout });
    ...
    await expect(reportDiv).toContainText(/Matched Skills \(\d+\):/i, { timeout: 10000 });
    ...
    for (const skill of expectedSkills) {
      try {
        await expect(reportDiv).toContainText(new RegExp(skill, 'i'), { timeout: 5000 });
        skillsFound++;
    ...
    expect(skillsFound, `Expected at least 3 skills from multi-page PDF, found: ${skillsFound}`).toBeGreaterThanOrEqual(3);

    console.log(`🎯 Multi-page PDF test completed. Skills extracted: ${skillsFound}/${expectedSkills.length}`);
  });

  test('Image-based PDF Test: Graceful error handling for unreadable content', async ({ page }) => {
    // Fix path resolution - tests run from e2e directory context
    const jdPath = path.resolve('UAT_Architect_JD.txt');
    ...
    // Verify test artifacts exist
    expect(fs.existsSync(jdPath), `JD file not found: ${jdPath}`).toBeTruthy();
    expect(fs.existsSync(imagePdfPath), `Image-only PDF not found: ${imagePdfPath}`).toBeTruthy();
    ...
    try {
      ...
      console.log('⚠️ API response timeout, but continuing with image-only PDF test');
    }
    ...
    await expect(reportDiv).toContainText(/Matched Skills \(0\):/i, { timeout: 10000 });
    await expect(reportDiv).toContainText(/Missing Skills \(0\):/i, { timeout: 10000 });
    
    // Check that "None" appears for empty results
    await expect(reportDiv).toContainText(/None/i, { timeout: 5000 });

    console.log('✅ Image-based PDF correctly shows empty results due to text extraction failure');
    console.log('✅ Frontend gracefully handles unreadable PDF by showing empty skills instead of crashing');

    console.log('🎯 Image-based PDF test completed - proper empty results handling validated');
  });

  test('PDF Processing Edge Cases: File validation and error boundaries', async ({ page }) => {
    ...
    if (isDisabledWithoutFile) {
      ...
      if (!hasValidation) {
        const hasResults = await page.locator('text=/diagnostic report|analysis result|分析结果/i').count() > 0;
        if (!hasResults) {
          console.log('✅ Form submission prevented without validation message');
      ...
      if (hasValidation) {
        ...
        console.log('⚠️ No validation detected, but form may handle this differently');
      }
    }
    ...
    try {
      ...
      const hasFileTypeError = await page.locator('text=/invalid file type|invalid format|only pdf|仅支持pdf/i').count() > 0;
      if (hasFileTypeError) {
