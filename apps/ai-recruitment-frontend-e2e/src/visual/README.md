...
expect: {
  toHaveScreenshot: {
    maxDiffPixels: 100,
    threshold: 0.2,
    animations: 'disabled',  // 禁用动画以获得一致的结果
  },
}
```
...
为提高测试稳定性，所有视觉测试已添加以下改进：

1. **禁用动画** - 使用 CSS 注入禁用所有动画和过渡效果
2. **等待页面稳定** - 使用 `domcontentloaded` 和 `networkidle` 事件
3. **等待图片加载** - 检查所有图片是否加载完成
4. **添加超时** - 每个截图操作添加 15 秒超时
5. **元素稳定等待** - 截图前等待 500ms 确保元素渲染稳定
