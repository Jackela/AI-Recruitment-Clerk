...
import { stableNavigation, stableElementCheck, checkConnectionHealth, BROWSER_CONFIGS } from '../connection-stability';

test.describe('Essential Cross-Browser Compatibility', () => {
  
  test('Browser can establish basic connection', async ({ page, browserName }) => {
    console.log(`🔌 Testing ${browserName} basic connection...`);
    
    // Use connection health check utility
    const isHealthy = await checkConnectionHealth(page, 'http://localhost:4202/', browserName);
    
    if (!isHealthy) {
      console.log(`⚠️ Initial health check failed for ${browserName}, attempting recovery...`);
      
      // Fallback: simple navigation test
      try {
        await page.goto('http://localhost:4202/', { 
          waitUntil: 'domcontentloaded', 
          timeout: BROWSER_CONFIGS[browserName as keyof typeof BROWSER_CONFIGS]?.navigationTimeout || 45000
        });
  ...
  test('Browser can load application title', async ({ page, browserName }) => {
    ...
    await stableNavigation(page, 'http://localhost:4202/', {
      waitUntil: 'domcontentloaded',
      retries: 2
    }, browserName);
    
    // Use stable element check utility
    const titleFound = await stableElementCheck(page, '#app-title', {
      maxRetries: 3,
      timeoutMs: 20000
    }, browserName);
    ...
    if (titleFound) {
      ...
      console.log(`⚠️ ${browserName} specific title not found, but page content loaded`);
    }
  });
  
  test('Browser supports basic JavaScript execution', async ({ page, browserName }) => {
    ...
    try {
      ...
      console.log(`   Document state: ${result.documentReady}, Title: "${result.title}"`);
      
    } catch (error) {
      console.log(`⚠️ ${browserName} JavaScript evaluation failed: ${error.message}`);
  ...
  test('Browser handles single route navigation', async ({ page, browserName }) => {
    ...
    try {
      ...
      console.log(`✅ ${browserName} maintained page state despite navigation issue`);
    }
  });
});