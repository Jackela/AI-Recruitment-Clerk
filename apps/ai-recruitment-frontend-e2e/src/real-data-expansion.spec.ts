...
function validateTestPrerequisites(): string {
  try {
    ...
    console.log(`✅ JD file validated: ${path.basename(PATHS.jdFile)} (${jdContent.length} characters)`);
    return jdContent;
  } catch (error) {
...
function logTestContext(filename: string, stage: string, details?: any) {
  ...
  console.log(`[${timestamp}] 🔍 ${filename} | ${stage}${details ? ` | ${JSON.stringify(details)}` : ''}`);
}

/**
 * Comprehensive UAT workflow execution
 */
async function executeUATWorkflow(page: any, filename: string, jdContent: string) {
  ...
  try {
    ...
    if (navigationTime > PERFORMANCE_THRESHOLDS.maxLoadTime) {
      console.warn(`⚠️ Slow navigation: ${navigationTime}ms (threshold: ${PERFORMANCE_THRESHOLDS.maxLoadTime}ms)`);
    }

    logTestContext(filename, 'Navigation completed', { navigationTime });

    // Step 2: Verify coach page loaded correctly
    await expect(page.locator('h2').filter({ hasText: /coach.*analysis|coach.*gap|gap.*analysis/i }))
    ...
    expect(fs.existsSync(resumePath), `Resume file not found: ${resumePath}`).toBeTruthy();
    const fileStats = fs.statSync(resumePath);
    expect(fileStats.size, `Resume file is empty: ${filename}`).toBeGreaterThan(0);
    ...
    await expect(analyzeButton).toBeEnabled({ timeout: TEST_CONFIG.retryDelay });

    // Monitor API requests
    const analysisStart = Date.now();
    const responsePromise = page.waitForResponse(
      response => response.url().includes('/gap-analysis-file') || response.url().includes('/coach/analyze'),
    ...
    try {
      ...
      if (analysisTime > PERFORMANCE_THRESHOLDS.maxAnalysisTime) {
        console.warn(`⚠️ Slow analysis: ${analysisTime}ms (threshold: ${PERFORMANCE_THRESHOLDS.maxAnalysisTime}ms)`);
      }
      ...
      logTestContext(filename, 'API response timeout - continuing with UI validation');
      console.warn(`⚠️ API timeout for ${filename}, but continuing with UI validation`);
    }
    ...
    await expect(reportComponent).toBeVisible({ timeout: TEST_CONFIG.baseTimeout });

    // Verify diagnostic report heading
    const diagnosticHeading = page.locator('h3').filter({ hasText: /diagnostic report/i });
    await expect(diagnosticHeading).toBeVisible({ timeout: TEST_CONFIG.baseTimeout });
    ...
    const hasMatchedSkills = await reportDiv.locator('text=/Matched Skills.*:/i').count() > 0;
    const hasMissingSkills = await reportDiv.locator('text=/Missing Skills.*:/i').count() > 0;

    expect(hasMatchedSkills || hasMissingSkills, 
      'Either matched or missing skills section should be visible').toBeTruthy();
...
test.describe('Real Data Expansion - Dynamic PDF Resume Testing', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('request', (request) => {
      if (request.url().includes('/coach') || request.url().includes('/gap-analysis')) {
        console.log(`📤 Request: ${request.method()} ${request.url()}`);
      }
    });

    page.on('response', (response) => {
      if (response.url().includes('/coach') || response.url().includes('/gap-analysis')) {
  ...
  for (const filename of resumeFiles) {
    ...
    test(`Dynamic UAT: ${testName} (${filename})`, async ({ page }) => {
      ...
      console.log(`📊 File ${resumeFiles.indexOf(filename) + 1} of ${resumeFiles.length}`);
      ...
      try {
        ...
        try {
          ...
          console.warn('Failed to capture failure screenshot:', screenshotError.message);
        }
  ...
  test('Test Suite Summary and Validation', async ({ page }) => {
    ...
    console.log(`📊 Performance thresholds: ${JSON.stringify(PERFORMANCE_THRESHOLDS, null, 2)}`);
    
    // Verify test data integrity
    for (const filename of resumeFiles) {
      const resumePath = path.join(PATHS.resumesDir, filename);
      const fileStats = fs.statSync(resumePath);
      expect(fileStats.size, `File ${filename} should not be empty`).toBeGreaterThan(0);
    }
    
    console.log(`✅ All ${resumeFiles.length} PDF files validated for integrity`);
