...
export async function stableNavigation(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🌐 Navigation attempt ${attempt}/${maxRetries} to ${url} [${browserName}]`,
      );
      ...
      console.log(
        `✅ Navigation successful on attempt ${attempt} [${browserName}]`,
      );
      return;
    } catch (error) {
      lastError = error as Error;
      console.log(
        `⚠️ Navigation attempt ${attempt} failed: ${(error as Error).message} [${browserName}]`,
      );
...
export async function stableElementCheck(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `🔍 Element check attempt ${attempt}/${maxRetries} for ${selector} [${browserName}]`,
      );
      ...
      console.log(
        `⚠️ Element check attempt ${attempt} failed: ${(error as Error).message} [${browserName}]`,
      );
...
export async function stableEvaluate<T>(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(
        `⚡ JavaScript evaluation attempt ${attempt}/${maxRetries} [${browserName}]`,
      );

      const result = await page.evaluate(evaluateFunction);

      console.log(
        `✅ JavaScript evaluation successful on attempt ${attempt} [${browserName}]`,
      );
      return result;
    } catch (error) {
      lastError = error as Error;
      console.log(
        `⚠️ JavaScript evaluation attempt ${attempt} failed: ${(error as Error).message} [${browserName}]`,
      );
  ...
  throw new Error(
    `JavaScript evaluation failed after ${maxRetries} attempts. Last error: ${lastError?.message}`,
  );
}
...
export async function checkConnectionHealth(
  ...
  try {
    ...
    const bodyVisible = await stableElementCheck(
      page,
      'body',
      {
        maxRetries: 2,
    ...
    if (bodyVisible) {
      ...
      console.log(
        `❌ Connection health check failed - no body element [${browserName}]`,
      );
      return false;
    }
  } catch (error) {
    console.log(
      `❌ Connection health check failed: ${(error as Error).message} [${browserName}]`,
    );
    return false;
  }
}
...
export async function measurePerformance(
  ...
  try {
    console.log(
      `📊 Starting performance measurement for ${operationName} [${browserName}]`,
    );
