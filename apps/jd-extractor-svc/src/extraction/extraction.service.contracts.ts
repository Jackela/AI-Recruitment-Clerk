...
export class ExtractionServiceContracts {
  ...
  async extractJobRequirements(
    ...
    try {
      ...
      try {
        ...
        this.logger.warn(
          `LLM extraction failed, attempting fallback analysis: ${llmError.message}`,
        );
      ...
      const processingTime = Date.now() - startTime;
      this.logger.error(
        `JD extraction failed after ${processingTime}ms:`,
        error,
      );
  ...
  async handleJobJdSubmitted(event: JobJdSubmittedEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.log(
        `Successfully processed and published JD extraction for jobId: ${jobId}`,
      );
    } catch (error) {
  ...
  private async validateAndEnhanceLLMResponse(
    ...
    if (result.requiredSkills.length === 0) {
      result.requiredSkills = this.extractSkillsUsingRules(originalText).map(
        (skill) => ({
          skill,
  ...
  private extractExperienceUsingRules(text: string): {
    min: number;
    max: number;
  } {
  ...
  private extractEducationUsingRules(
    text: string,
  ...
  private inferSeniorityFromText(
    text: string,
  ...
  private async handleProcessingError(
    ...
    // Publish error event
    try {
      await this.natsClient.publishProcessingError(
        jobId,
