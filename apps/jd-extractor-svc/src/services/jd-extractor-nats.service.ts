...
export class JdExtractorNatsService extends NatsClientService {
  ...
  async publishExtractionStarted(event: {
    ...
    try {
      this.logger.log(`📤 Publishing extraction started event for jobId: ${event.jobId}`);
      ...
      if (result.success) {
        this.logger.log(`✅ Extraction started event published - JobId: ${event.jobId}, MessageId: ${result.messageId}`);
      } else {
        this.logger.error(`❌ Failed to publish extraction started event - JobId: ${event.jobId}, Error: ${result.error}`);
      }
      
      return result;
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      this.logger.error(`❌ Error publishing extraction started event for jobId: ${event.jobId}`, error);
