...
describe('🚀 API Performance and Load Testing', () => {
  ...
  beforeAll(async () => {
    ...
    console.log('📊 Monitoring: Response Times, Memory Usage, Concurrent Requests');
  });
  ...
  async function measurePerformance(
    ...
    try {
      ...
      performanceMetrics.responseTime[category as keyof typeof performanceMetrics.responseTime].push(responseTime);
  ...
  async function generatePerformanceReport() {
    ...
    Object.entries(performanceMetrics.responseTime).forEach(([category, times]) => {
      if (times.length > 0) {
    ...
    const totalMemoryUsage = performanceMetrics.memoryUsage.reduce((a, b) => a + Math.abs(b), 0);
    console.log(`\\n💾 MEMORY USAGE:`);
    console.log(`   Total memory delta: ${(totalMemoryUsage / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   Average per request: ${(totalMemoryUsage / performanceMetrics.requestCount / 1024).toFixed(2)}KB`);
    
    console.log(`\\n📊 OVERALL STATS:`);
    console.log(`   Total requests: ${performanceMetrics.requestCount}`);
    console.log(`   Total errors: ${performanceMetrics.errorCount}`);
    console.log(`   Success rate: ${((performanceMetrics.requestCount - performanceMetrics.errorCount) / performanceMetrics.requestCount * 100).toFixed(2)}%`);
  }
  ...
  describe('🔐 Authentication Performance', () => {
    ...
    it('should handle registration load efficiently', async () => {
      ...
      const successfulRegistrations = results.filter(r => 
        r.result && [200, 201].includes(r.result.status)
      );
  ...
  describe('👥 User Management Performance', () => {
    it('should handle profile requests under load', async () => {
      ...
      const avgResponseTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(`📊 Average profile fetch time: ${avgResponseTime.toFixed(2)}ms`);
    });
  ...
  describe('📄 Resume Processing Performance', () => {
    it('should handle multiple resume uploads concurrently', async () => {
        ...
        uploadRequests.push(
          measurePerformance('resumeProcessing', async () => {
            return await request(app.getHttpServer())
              .post('/resumes/upload')
              .set('Authorization', `Bearer ${userToken}`)
              .attach('resume', Buffer.from(`Mock Resume Content ${i}`), `performance-resume-${i}.pdf`)
      ...
      const avgResponseTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(`📊 Average resume upload time: ${avgResponseTime.toFixed(2)}ms`);
    });
  ...
  describe('📊 Questionnaire Performance', () => {
    ...
    it('should handle questionnaire creation efficiently', async () => {
      const { result, responseTime } = await measurePerformance('questionnaire', async () => {
        return await request(app.getHttpServer())
          .post('/questionnaire')
          .set('Authorization', `Bearer ${adminToken}`)
          .send({
    ...
    it('should handle multiple questionnaire submissions', async () => {
      if (!performanceQuestionnaireId) {
        console.log('⚠️ Skipping questionnaire submissions - no questionnaire ID');
        return;
      }
        ...
        submissionRequests.push(
          measurePerformance('questionnaire', async () => {
            const responses = Array.from({ length: 20 }, (_, qIndex) => ({
              ...
              answer: ['Option A', 'Option B', 'Option C'][Math.floor(Math.random() * 3)]
            }));
    ...
    it('should handle questionnaire list requests under load', async () => {
      ...
      const avgResponseTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(`📊 Average questionnaire list time: ${avgResponseTime.toFixed(2)}ms`);
    });
  });
  ...
  describe('📈 Analytics Performance', () => {
    it('should handle analytics event tracking efficiently', async () => {
      ...
      const avgResponseTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(`📊 Average event tracking time: ${avgResponseTime.toFixed(2)}ms`);
  ...
  describe('🔧 System Performance', () => {
    it('should handle health check requests efficiently', async () => {
      ...
      const avgResponseTime = results.reduce((sum, r) => sum + r.responseTime, 0) / results.length;
      console.log(`📊 Average health check time: ${avgResponseTime.toFixed(2)}ms`);
  ...
  describe('🔥 Stress Testing and Breaking Points', () => {
    it('should identify rate limiting thresholds', async () => {
      ...
      const successRequests = results.filter(r => r.result && r.result.status === 200);
      const rateLimitedRequests = results.filter(r => r.result && r.result.status === 429);
      
      console.log(`✅ Successful: ${successRequests.length}`);
      console.log(`⚠️ Rate limited: ${rateLimitedRequests.length}`);
      console.log(`❌ Errors: ${results.length - successRequests.length - rateLimitedRequests.length}`);
    ...
    it('should handle memory pressure scenarios', async () => {
      ...
      results.forEach((result, index) => {
        console.log(`Export ${index + 1}: ${result.responseTime}ms, Memory: ${(result.memoryDelta / 1024).toFixed(2)}KB`);
  ...
  describe('📊 Performance Regression Detection', () => {
    it('should validate performance benchmarks', async () => {
      ...
      // Login benchmark
      const { responseTime: loginTime } = await measurePerformance('auth', async () => {
        return await request(app.getHttpServer())
          .post('/auth/login')
          .send({
            email: 'performance.user@test.com',
      ...
      // Profile benchmark
      const { responseTime: profileTime } = await measurePerformance('userManagement', async () => {
        return await request(app.getHttpServer())
          .get('/users/profile')
          .set('Authorization', `Bearer ${userToken}`);
      });
      ...
      const { responseTime: healthTime } = await measurePerformance('system', async () => {
        return await request(app.getHttpServer())
          .get('/system/health');
      });
      ...
      Object.entries(benchmarkResults).forEach(([test, time]) => {
        ...
        console.log(`   ${test}: ${time}ms (threshold: ${threshold}ms) ${status}`);
      });
    });
  });
  ...
  describe('📋 Performance Test Summary', () => {
    it('should generate comprehensive performance report', async () => {
      ...
      if (allResponseTimes.length > 0) {
        ...
        console.log(`   Error rate: ${(performanceMetrics.errorCount / performanceMetrics.requestCount * 100).toFixed(2)}%`);
        
        // Performance assertions
        expect(overallAvg).toBeLessThan(3000); // Overall average < 3 seconds
        expect(performanceMetrics.errorCount / performanceMetrics.requestCount).toBeLessThan(0.05); // < 5% error rate
