...
describe('🛡️ Rate Limiting & DDoS Protection Security Tests', () => {
  ...
  async function testRateLimit(
    ...
    for (let i = 0; i < requestCount; i++) {
      ...
      if (requestData) {
        requestBuilder = (requestBuilder as any).send(requestData);
      }
    ...
    return {
      ...
      successCount: responses.filter((r) => [200, 201, 202].includes(r.status))
        .length,
  ...
  describe('⏱️ Basic Rate Limiting', () => {
    it('should enforce per-IP rate limits on public endpoints', async () => {
      ...
      console.log(
        `Public endpoint test: ${result.successCount}/${burstSize} succeeded, ${result.rateLimitedCount} rate limited`,
      );
      ...
      const rateLimitedResponse = result.responses.find(
        (r) => r.status === 429,
      );
      if (rateLimitedResponse) {
        ...
        expect(
          parseInt(rateLimitedResponse.headers['x-ratelimit-remaining']),
        ).toBe(0);
      }
    });
    ...
    it('should enforce per-user rate limits on authenticated endpoints', async () => {
      ...
      console.log(
        `User endpoint test: ${result.successCount}/${burstSize} succeeded, ${result.rateLimitedCount} rate limited`,
      );
    ...
    it('should enforce stricter limits on resource-intensive operations', async () => {
      ...
      console.log(
        `Intensive endpoint test: ${result.successCount}/10 succeeded, ${result.rateLimitedCount} rate limited`,
      );
    ...
    it('should implement sliding window rate limiting', async () => {
      ...
      expect(
        secondBurst.successCount + firstBurst.rateLimitedCount,
      ).toBeGreaterThan(firstBurst.successCount);
    });
  });
  ...
  describe('🚨 DDoS Attack Simulation', () => {
    it('should mitigate volumetric DDoS attacks', async () => {
      ...
      console.log(
        `DDoS simulation: ${successful.length} successful, ${rateLimited.length} rate limited, ${serverErrors.length} errors`,
      );
    ...
    it('should detect and mitigate slow DoS attacks', async () => {
      ...
      console.log(
        `Slow DoS test: ${successful.length} successful, ${rateLimited.length} rate limited`,
      );
    ...
    it('should handle application-layer DDoS attacks', async () => {
      ...
      console.log(
        `Complex query test: ${successful.length} successful, ${rateLimited.length} rate limited, ${badRequest.length} rejected`,
      );
  ...
  describe('🔄 Adaptive Rate Limiting', () => {
    it('should adjust limits based on system load', async () => {
      ...
      const testDuringLoad = await testRateLimit(
        '/users/profile',
        'GET',
        userToken,
        20,
      ...
      console.log(
        `During high load: ${testDuringLoad.successCount}/20 succeeded, ${testDuringLoad.rateLimitedCount} rate limited`,
      );
    ...
    it('should implement intelligent request categorization', async () => {
      ...
      const criticalSuccess = criticalResponses.filter(
        (r) => r.status === 200,
      ).length;
      const normalSuccess = normalResponses.filter(
        (r) => r.status === 200,
      ).length;
      const normalRateLimited = normalResponses.filter(
        (r) => r.status === 429,
      ).length;

      console.log(`Critical requests: ${criticalSuccess}/10 succeeded`);
      console.log(
        `Normal requests: ${normalSuccess}/20 succeeded, ${normalRateLimited} rate limited`,
      );
  ...
  describe('📊 Request Pattern Analysis', () => {
    it('should detect suspicious request patterns', async () => {
      ...
      console.log(
        `Suspicious pattern test: Status codes [${statusCodes.join(', ')}]`,
      );
    ...
    it('should implement anomaly detection for request timing', async () => {
      ...
      console.log(
        `Anomaly detection test: ${rateLimited}/15 requests rate limited`,
      );
  ...
  describe('🚫 IP-Based Protection', () => {
    ...
    it('should handle distributed attacks across IPs', async () => {
      ...
      console.log(
        `Distributed attack test: ${successful} successful, ${rateLimited} rate limited`,
      );
  ...
  describe('⚡ Burst Protection', () => {
    it('should implement token bucket algorithm', async () => {
      ...
      const firstBurst = await testRateLimit(
        endpoint,
        'POST',
        userToken,
        10,
      ...
      console.log(
        `Token bucket first burst: ${firstBurst.successCount}/10 succeeded`,
      );

      // Immediate second burst - should be limited
      const secondBurst = await testRateLimit(
        endpoint,
        'POST',
        userToken,
        10,
      ...
      console.log(
        `Token bucket second burst: ${secondBurst.successCount}/10 succeeded`,
      );
      ...
      const thirdBurst = await testRateLimit(
        endpoint,
        'POST',
        userToken,
        5,
      ...
      console.log(
        `Token bucket after refill: ${thirdBurst.successCount}/5 succeeded`,
      );
    ...
    it('should handle burst traffic gracefully', async () => {
      ...
      console.log(
        `Burst test: ${successful} successful, ${rateLimited} rate limited in ${duration}ms`,
      );
  ...
  describe('🔧 Rate Limit Configuration', () => {
    it('should support different limits for different endpoints', async () => {
      ...
      for (const test of endpointTests) {
        ...
        console.log(
          `${test.endpoint} (${test.expected}): ${result.successCount}/15 succeeded, ${result.rateLimitedCount} rate limited`,
        );
      }
  ...
  describe('📋 Rate Limiting Security Summary', () => {
    it('should validate comprehensive rate limiting security', async () => {
      ...
      const securityProtections = {
        basicRateLimiting:
          '✅ Per-IP and per-user rate limiting with proper headers',
        ddosProtection:
          '✅ Volumetric, slow DoS, and application-layer DDoS mitigation',
        adaptiveRateLimiting:
          '✅ System load-based adaptive limits and request prioritization',
        patternAnalysis: '✅ Suspicious request pattern and anomaly detection',
        ipProtection:
          '✅ IP-based rate limiting and distributed attack handling',
        burstProtection:
          '✅ Token bucket algorithm and burst traffic management',
        configurationFlexibility:
          '✅ Endpoint-specific limits and informative headers',
      };
