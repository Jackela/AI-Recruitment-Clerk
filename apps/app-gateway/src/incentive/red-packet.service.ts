...
export class RedPacketService {
  ...
  async processQuestionnaireReward(data: {
    ...
    try {
      ...
      if (sendResult.success) {
        ...
        await this.updateRedPacketStatus(redPacket.id, 'failed', undefined, sendResult.error);
  ...
  async processReferralReward(data: {
    ...
    try {
      ...
      return {
        ...
        message: referrerPacket.success && refereePacket.success 
  ...
  async getRedPacketStats(date?: string): Promise<{
    ...
    try {
      ...
      const breakdown = {
        questionnaire: {
          count: records.filter(r => r.type === 'questionnaire' && r.status === 'sent').length,
          amount: records
            .filter(r => r.type === 'questionnaire' && r.status === 'sent')
            .reduce((sum, r) => sum + r.amount, 0)
        },
        qualityBonus: {
          count: records.filter(r => r.amount > this.config.baseAmount && r.status === 'sent').length,
          amount: records
            .filter(r => r.amount > this.config.baseAmount && r.status === 'sent')
            .reduce((sum, r) => sum + (r.amount - this.config.baseAmount), 0)
        },
        referral: {
          count: records.filter(r => r.type === 'referral' && r.status === 'sent').length,
          amount: records
      ...
      return {
        ...
        budgetUsage: { used: 0, remaining: this.config.dailyBudget, percentage: 0 }
      };
    }
  }
  ...
  private async sendWeChatRedPacket(redPacket: RedPacketRecord): Promise<{
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        resolve({
          success,
          transactionId: success ? `wx_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : undefined,
  ...
  private async sendAlipayRedPacket(redPacket: RedPacketRecord): Promise<{
    ...
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        resolve({
          success,
          transactionId: success ? `alipay_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : undefined,
  ...
  private async updateRedPacketStatus(
    ...
    // TODO: 更新数据库记录状态
    console.log('Updating red packet status:', { id, status, sentAt, failureReason });
  }
  ...
  private async getRedPacketRecordsByDate(date: string): Promise<RedPacketRecord[]> {
