...
describe('JobRepository Integration', () => {
  ...
  beforeAll(async () => {
    ...
    module = await Test.createTestingModule({
      imports: [
        MongooseModule.forRoot(memoryUri),
      ...
      providers: [
        JobRepository,
        {
          ...
          useValue: {
            generateKey: jest.fn((...args: any[]) => args.join(':')),
  ...
  describe('CRUD Operations', () => {
    ...
    const testJobData = {
      ...
      description:
        '我们正在寻找一位经验丰富的Node.js开发工程师，负责后端API开发和系统架构设计。',
      requirements: [
    ...
    it('should find all jobs with default options', async () => {
      ...
      expect(jobs.some((job) => job._id.toString() === createdJobId)).toBe(
        true,
      );
    });
    ...
    it('should find jobs by creator', async () => {
      const jobsByCreator = await repository.findByCreatedBy(
        testJobData.createdBy,
      );
    ...
    it('should update JD analysis', async () => {
      ...
      const updatedJob = await repository.updateJdAnalysis(
        createdJobId,
        keywords,
        confidence,
      );
    ...
    it('should find jobs by skills', async () => {
      const jobsWithSkills = await repository.findBySkills([
        'Node.js',
        'TypeScript',
      ]);
    ...
    it('should count jobs by company', async () => {
      ...
      expect(
        companyCounts.some((item) => item.company === testJobData.company),
      ).toBe(true);
    });
  ...
  describe('Search Operations', () => {
    ...
    it('should search jobs by keywords', async () => {
      ...
      try {
        const searchResults = await repository.searchByKeywords([
          'Python',
          '数据',
        ]);
