...
export class NotificationService {
  ...
  public async sendNotification(notification: NotificationData): Promise<void> {
    ...
    if (!this.shouldSendNotification(notification, prefs)) {
      this.logger.debug(
        `Notification filtered by user preferences: ${notification.id}`,
      );
      return;
    }
    ...
    this.logger.log(
      `Notification ${notification.id} sent to user ${notification.userId}`,
    );
  }
  ...
  public async broadcastToRoom(
    ...
    await this.cacheService.set(`broadcast:${message.id}`, message, {
      ttl: 3600,
    }); // 1 hour TTL

    // In a real implementation, emit to all users in the room
    this.logger.debug(
      `Broadcast message ${message.id} stored for room ${roomId}`,
    );
  }
  ...
  public async systemBroadcast(message: BroadcastMessage): Promise<void> {
    ...
    await this.cacheService.set(`system_broadcast:${message.id}`, message, {
      ttl: 7200,
    }); // 2 hours TTL
  ...
  public async updateUserPreferences(
    ...
    await this.cacheService.set(`notification_prefs:${userId}`, updatedPrefs, {
      ttl: 86400,
    }); // 24 hours TTL
  ...
  public async createFromTemplate(
    ...
    const notification: NotificationData = {
      ...
      actionUrl: template.actionUrl
        ? this.interpolateTemplate(template.actionUrl, templateData)
        : undefined,
      actionText: template.actionText,
      expiresAt: template.expiresIn
        ? new Date(Date.now() + template.expiresIn * 1000)
        : undefined,
    };
  ...
  private async cacheUserNotifications(
    ...
    try {
      await this.cacheService.set(`notifications:${userId}`, notifications, {
        ttl: 86400,
      }); // 24 hours TTL
    } catch (error) {
  ...
  private getNotificationTemplate(templateType: string): any {
    ...
    const templates: Record<string, any> = {
      analysis_complete: {
        ...
        message:
          'Your resume analysis for {{candidateName}} has been completed.',
        priority: 'normal',
        persistent: false,
  ...
  public async cleanupExpiredData(): Promise<void> {
    ...
    for (const [userId, notifications] of this.userNotifications.entries()) {
      const filteredNotifications = notifications.filter(
        (n) => n.createdAt > thirtyDaysAgo || n.persistent,
      );
    ...
    if (cleanedCount > 0) {
      this.logger.log(
        `Cleaned up ${cleanedCount} expired notifications and broadcasts`,
      );
    }
  }
}
