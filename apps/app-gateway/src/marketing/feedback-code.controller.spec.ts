...
describe('FeedbackCodeController', () => {
  ...
  describe('POST /record', () => {
    ...
    it('应该成功记录反馈码', async () => {
      ...
      const result = await controller.recordFeedbackCode(
        createDto,
        mockRequest as any,
      );
    ...
    it('应该验证反馈码格式', async () => {
      ...
      await expect(
        controller.recordFeedbackCode(invalidDto, mockRequest as any),
    ...
    it('应该处理服务层错误', async () => {
      ...
      await expect(
        controller.recordFeedbackCode(createDto, mockRequest as any),
  ...
  describe('GET /validate/:code', () => {
    ...
    it('应该拒绝格式无效的反馈码', async () => {
      ...
      await expect(
        controller.validateFeedbackCode(invalidCode),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it('应该处理服务层错误', async () => {
      ...
      await expect(controller.validateFeedbackCode(testCode)).rejects.toThrow(
        'Service error',
      );
    });
  });
  ...
  describe('POST /mark-used', () => {
    ...
    it('应该验证必填字段', async () => {
      ...
      await expect(controller.markAsUsed(incompleteDto as any)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('应该验证支付宝账号格式', async () => {
      ...
      await expect(controller.markAsUsed(invalidDto)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('应该处理无效或已使用的反馈码', async () => {
      ...
      await expect(controller.markAsUsed(markUsedDto)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('POST /webhook/questionnaire', () => {
    ...
    it('应该处理不完整的webhook数据', async () => {
      ...
      const result =
        await controller.handleQuestionnaireWebhook(incompleteData);
    ...
    it('应该支持不同的数据格式', async () => {
      ...
      const result =
        await controller.handleQuestionnaireWebhook(alternativeFormat);
  ...
  describe('辅助方法测试', () => {
    ...
    it('应该验证有效的支付宝账号格式', () => {
      ...
      expect((controller as any).isValidAlipayAccount('13812345678')).toBe(
        true,
      );
