...
describe('FeedbackCodeService', () => {
  ...
  beforeEach(async () => {
    ...
    model = module.get<Model<FeedbackCodeDocument>>(
      getModelToken('FeedbackCode'),
    );
  });
  ...
  describe('recordFeedbackCode', () => {
    ...
    it('应该正确处理数据库错误', async () => {
      ...
      await expect(
        service.recordFeedbackCode(createDto, metadata),
  ...
  describe('markAsUsed', () => {
    ...
    it('应该处理无效或已使用的反馈码', async () => {
      ...
      await expect(service.markAsUsed(markUsedDto)).rejects.toThrow(
        '反馈码无效或已使用',
      );
    });
  });
  ...
  describe('updatePaymentStatus', () => {
    ...
    it('应该处理不存在的反馈码', async () => {
      ...
      await expect(
        service.updatePaymentStatus(testCode, 'paid'),
  ...
  describe('batchUpdatePaymentStatus', () => {
    ...
    it('应该批量更新支付状态', async () => {
      ...
      const result = await service.batchUpdatePaymentStatus(
        testCodes,
        'paid',
        '批量处理',
      );
  ...
  describe('质量评分算法', () => {
    it('应该为高质量反馈给出高分', () => {
      const highQualityData = {
        problems:
          '系统响应速度有时候比较慢，特别是在处理大文件时需要等待较长时间',
        favorite_features:
          '我最喜欢AI简历解析功能，因为它能够准确识别和提取关键信息，大大提高了工作效率',
        improvements:
          '建议增加批量处理功能，优化系统响应速度，改进用户界面设计使其更加直观',
