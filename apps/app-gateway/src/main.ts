...
async function bootstrap() {
  ...
  if (process.env.NODE_ENV !== 'test' && missingVars.length > 0) {
    Logger.warn('⚠️ [FAIL-FAST] Some env vars missing at bootstrap (will rely on ConfigModule .env):');
    for (const v of missingVars) Logger.warn(`   • ${v} is not set at process.env yet`);
    Logger.warn('   If .env exists at repo root, Nest ConfigModule will load it shortly.');
  }
