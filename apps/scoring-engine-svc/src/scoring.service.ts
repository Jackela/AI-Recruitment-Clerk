...
export class ScoringEngineService {
  ...
  async handleResumeParsedEvent(event: {
    ...
    try {
      ...
      const score = await this._calculateEnhancedMatchScore(
        jdDto,
        event.resumeDto,
      );
      ...
      const scoringError =
        error instanceof ScoringEngineException
          ? error
   ...
   */
  protected async _calculateEnhancedMatchScore(
    jdDto: JdDTO,
    resumeDto: ResumeDTO,
  ): Promise<ScoreDTO> {
    const startTime = Date.now();
    ...
    try {
      ...
      const weights = this._calculateDynamicWeights(
        jdDto,
      ...
      const aiAnalysisTime = processingMetrics.aiResponseTimes.reduce(
        (a, b) => a + b,
        0,
      );
  ...
  private _calculateDynamicWeights(
    ...
    if (
      jdDto.seniority === 'senior' ||
      jdDto.seniority === 'lead' ||
      jdDto.seniority === 'executive'
    ) {
   ...
   */
  private _calculateEnhancedEducationScore(
    resumeDto: ResumeDTO,
    jdDto: JdDTO,
  ): ScoreComponent {
    ...
    // Base score
    let score =
      highestEducation >= requiredLevel
        ? 1.0
        : highestEducation / Math.max(requiredLevel, 1);
  ...
  protected _calculateMatchScore(jdDto: JdDTO, resumeDto: ResumeDTO): ScoreDTO {
    ...
    const totalYears =
      resumeDto.workExperience.reduce((acc, w) => {
    ...
    const overallScore = Math.round(
      (skillScoreValue * 0.5 +
        experienceScoreValue * 0.3 +
        educationScoreValue * 0.2) *
        100,
    );
