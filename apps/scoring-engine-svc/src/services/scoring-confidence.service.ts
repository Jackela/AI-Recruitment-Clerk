...
export class ScoringConfidenceService {
  ...
  generateConfidenceReport(
    ...
    try {
      ...
      const qualityIndicators = this.generateQualityIndicators(confidenceMetrics);
  ...
  private assessCompleteness(resume: ResumeDTO): number {
    ...
    const incompleteExperience = resume.workExperience.filter(exp => 
    ...
    const incompleteEducation = resume.education.filter(edu => 
  ...
  private assessConsistency(resume: ResumeDTO): number {
    ...
    const dateInconsistencies = this.checkDateConsistency(resume.workExperience);
    score -= dateInconsistencies * 10;
    
    // Position progression consistency
    const progressionIssues = this.checkProgressionConsistency(resume.workExperience);
  ...
  private assessRecency(resume: ResumeDTO): number {
    ...
    const mostRecent = resume.workExperience.reduce((latest, exp) => {
      const expEndDate = exp.endDate === 'present' ? new Date() : new Date(exp.endDate);
      const latestEndDate = latest.endDate === 'present' ? new Date() : new Date(latest.endDate);
      return expEndDate > latestEndDate ? exp : latest;
    });
    
    const endDate = mostRecent.endDate === 'present' ? new Date() : new Date(mostRecent.endDate);
    const monthsSinceLastJob = (new Date().getTime() - endDate.getTime()) / (1000 * 60 * 60 * 24 * 30);
  ...
  private identifyDataQualityIssues(factors: DataQualityFactors, resume: ResumeDTO): string[] {
    ...
    if (factors.completeness < 70) {
      issues.push('Missing critical information (contact, experience, or education)');
    }
  ...
  private checkDateConsistency(workExperience: ResumeDTO['workExperience']): number {
  ...
  private checkProgressionConsistency(workExperience: ResumeDTO['workExperience']): number {
    ...
    for (let i = 1; i < sorted.length; i++) {
      const prevEnd = sorted[i - 1].endDate === 'present' 
        ? new Date() 
  ...
  private checkSkillConsistency(resume: ResumeDTO): number {
    ...
    const alignedSkills = skillsLower.filter(skill => 
  ...
  private assessAnalysisReliability(
    ...
    const algorithmConfidence = Math.round(
      (componentScores.skills.confidence * 0.4 +
       ...
       componentScores.culturalFit.confidence * 0.25) * 100
    );
    ...
    const uncertainties = this.identifyAnalysisUncertainties(factors, processingMetrics);
  ...
  private assessAIResponseQuality(processingMetrics: ProcessingMetrics): number {
    ...
    const avgResponseTime = processingMetrics.aiResponseTimes.reduce((a, b) => a + b, 0) / 
    ...
    const fallbackRate = processingMetrics.fallbackUsed.filter(f => f).length / 
      processingMetrics.fallbackUsed.length;
    quality -= fallbackRate * 30;
    
    // Penalize for high error rates
    const avgErrorRate = processingMetrics.errorRates.reduce((a, b) => a + b, 0) / 
  ...
  private calculateScoreVariance(componentScores: ComponentScores): ScoreVarianceAssessment {
    ...
    const experienceVariance = (100 - componentScores.experience.confidence) / 10;
    const culturalFitVariance = (100 - componentScores.culturalFit.confidence) / 10;
    
    const overallVariance = Math.sqrt(
      (Math.pow(skillsVariance, 2) + 
       Math.pow(experienceVariance, 2) + 
       Math.pow(culturalFitVariance, 2)) / 3
    );
  ...
  private calculateRecommendationCertainty(
    ...
    const overallScore = Math.round(
      (scoringConsistency * 0.4 + dataCompleteness * 0.35 + algorithmMaturity * 0.25)
    );
    ...
    const riskFactors = this.identifyRiskFactors(dataQuality, analysisReliability, scoreVariance);
   ...
   */
  private calculateOverallConfidence(confidenceMetrics: ConfidenceMetrics): number {
    return Math.round(
  ...
  private generateConfidenceRecommendations(
    ...
    if (qualityIndicators.dataQualityGrade === 'C' || qualityIndicators.dataQualityGrade === 'D') {
      actionItems.push('Request additional resume information for more accurate scoring');
    }
    
    if (qualityIndicators.analysisDepthGrade === 'C' || qualityIndicators.analysisDepthGrade === 'D') {
      actionItems.push('Consider manual review to supplement algorithmic analysis');
    }
    
    if (qualityIndicators.reliabilityGrade === 'C' || qualityIndicators.reliabilityGrade === 'D') {
      actionItems.push('Use scoring results as initial filter only, conduct thorough interviews');
    }
    ...
    if (confidenceMetrics.dataQuality.score < 70) {
      riskMitigation.push('Verify critical information during interview process');
    }
    
    if (confidenceMetrics.scoreVariance.overallVariance > 1.5) {
      riskMitigation.push('Focus on areas with highest confidence scores');
      riskMitigation.push('Use range-based evaluation rather than point scores');
    }
   ...
   */
  private fallbackConfidenceReport(componentScores: ComponentScores): ScoreReliabilityReport {
    return {
