...
export class FieldMapperService {
  ...
  async normalizeToResumeDto(rawLlmOutput: any): Promise<ResumeDTO> {
    try {
      this.logger.debug('Starting normalization of raw LLM output to ResumeDTO');
      ...
      const contactInfo = await this.mapContactInfo(rawLlmOutput.contactInfo || {});
      
      // Normalize skills
      const skills = await this.normalizeSkills(rawLlmOutput.skills || []);
      
      // Normalize work experience
      const workExperience = await this.mapWorkExperience(rawLlmOutput.workExperience || []);
  ...
  async normalizeWithValidation(rawLlmOutput: any): Promise<FieldMappingResult> {
    try {
      ...
      const mappingConfidence = this.calculateMappingConfidence(resumeDto, validationErrors, rawLlmOutput);
      ...
      this.logger.debug(`Normalization with validation completed. Confidence: ${mappingConfidence}, Errors: ${validationErrors.length}`);
      return result;

    } catch (error) {
      this.logger.error('Failed to normalize with validation', error);
      throw new Error(`Normalization with validation failed: ${error.message}`);
    }
  }
  ...
  async validateResumeData(resumeDto: ResumeDTO): Promise<string[]> {
    ...
    try {
      ...
      if (!resumeDto.contactInfo) {
        ...
        if (!resumeDto.contactInfo.name || resumeDto.contactInfo.name.trim().length === 0) {
          errors.push('Contact name is missing or empty');
        }
        
        if (resumeDto.contactInfo.email && !this.isValidEmail(resumeDto.contactInfo.email)) {
          errors.push('Email format is invalid');
        }
        
        if (resumeDto.contactInfo.phone && !this.isValidPhone(resumeDto.contactInfo.phone)) {
  ...
  async mapWorkExperience(rawWorkExperience: any[]): Promise<ResumeDTO['workExperience']> {
    try {
      if (!Array.isArray(rawWorkExperience)) {
        this.logger.warn('Raw work experience is not an array, returning empty array');
        return [];
      }
      ...
      for (const rawExp of rawWorkExperience) {
        ...
        const summary = this.normalizeString(rawExp.summary || rawExp.description || '');
  ...
  async mapEducation(rawEducation: any[]): Promise<ResumeDTO['education']> {
    try {
      if (!Array.isArray(rawEducation)) {
        this.logger.warn('Raw education is not an array, returning empty array');
        return [];
      }
      ...
      for (const rawEdu of rawEducation) {
        ...
        // Extract and normalize fields
        const school = this.normalizeString(rawEdu.school || rawEdu.institution || rawEdu.university);
        const degree = this.normalizeDegree(rawEdu.degree || '');
        const major = this.normalizeString(rawEdu.major || rawEdu.field || rawEdu.fieldOfStudy);
    ...
    try {
      if (!Array.isArray(rawSkills)) {
        ...
        if (typeof rawSkills === 'string') {
          rawSkills = (rawSkills as string).split(/[,;\\n]/).map(s => s.trim()).filter(s => s.length > 0);
        } else {
          return [];
        }
      }
      ...
      this.logger.debug(`Normalized ${rawSkills.length} raw skills to ${limitedSkills.length} normalized skills across ${skillGroups.size} categories`);
   ...
   */
  async calculateExperience(workExperience: ResumeDTO['workExperience'], targetSkills?: string[]): Promise<{
    totalYears: number;
    ...
    try {
      const analysis = ExperienceCalculator.analyzeExperience(workExperience, targetSkills);
   ...
   */
  async extractDates(text: string): Promise<{ startDate: string; endDate: string; confidence: number }[]> {
    try {
      ...
      const extractedRanges: { startDate: string; endDate: string; confidence: number }[] = [];
      ...
      for (const pattern of dateRangePatterns) {
        ...
        while ((match = pattern.exec(text)) !== null) {
          ...
          if (startDate) {
            ...
            const confidence = Math.min(startParsed.confidence, endParsed.confidence);
   ...
   */
  private calculateMappingConfidence(resumeDto: ResumeDTO, validationErrors: string[], rawInput: any): number {
