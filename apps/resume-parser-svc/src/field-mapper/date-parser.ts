...
export class DateParser {
  private static readonly DATE_PATTERNS = [
    // ISO 8601 formats
    { pattern: /^(\d{4})-(\d{2})-(\d{2})$/, format: 'YYYY-MM-DD', confidence: 1.0 },
    { pattern: /^(\d{4})-(\d{2})$/, format: 'YYYY-MM', confidence: 0.9 },
    { pattern: /^(\d{4})$/, format: 'YYYY', confidence: 0.8 },
    
    // US formats
    { pattern: /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/, format: 'MM/DD/YYYY', confidence: 0.85 },
    { pattern: /^(\d{1,2})\/(\d{4})$/, format: 'MM/YYYY', confidence: 0.8 },
    
    // European formats
    { pattern: /^(\d{1,2})\.(\d{1,2})\.(\d{4})$/, format: 'DD.MM.YYYY', confidence: 0.85 },
    { pattern: /^(\d{1,2})\.(\d{4})$/, format: 'MM.YYYY', confidence: 0.8 },
    
    // Text-based months
    { pattern: /^(jan|january)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(feb|february)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(mar|march)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(apr|april)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(may)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(jun|june)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(jul|july)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(aug|august)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(sep|september)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(oct|october)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(nov|november)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    { pattern: /^(dec|december)\s+(\d{4})$/i, format: 'Month YYYY', confidence: 0.9 },
    
    // Full month names with day
    { pattern: /^(january|february|march|april|may|june|july|august|september|october|november|december)\s+(\d{1,2}),?\s+(\d{4})$/i, format: 'Month DD, YYYY', confidence: 0.95 },
    
    // Abbreviated month names
    { pattern: /^(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\.?\s+(\d{1,2}),?\s+(\d{4})$/i, format: 'Mon DD, YYYY', confidence: 0.9 },
  ...
  private static readonly PRESENT_KEYWORDS = [
    'present', 'current', 'now', 'ongoing', 'today', 'till date', 'till now', 'continuing'
  ];
  ...
  static parseDate(dateString: string): ParsedDate {
    ...
    if (!isNaN(fallbackDate.getTime()) && fallbackDate.getFullYear() > 1950 && fallbackDate.getFullYear() <= new Date().getFullYear() + 1) {
      return {
        date: fallbackDate,
        original: dateString,
        confidence: 0.6,
   ...
   */
  static calculateDuration(startDate: ParsedDate, endDate: ParsedDate): DateRange['duration'] {
   ...
   */
  static getDateBounds(ranges: DateRange[]): { earliest: Date | null; latest: Date | null } {
   ...
   */
  private static parseWithPattern(match: RegExpMatchArray, format: string): Date | null {
    try {
      switch (format) {
        case 'YYYY-MM-DD':
        ...
        case 'Month DD, YYYY':
          ...
          if (fullMonthNum) {
            return new Date(parseInt(match[3]), fullMonthNum - 1, parseInt(match[2]));
          }
          break;
        ...
        case 'Mon DD, YYYY':
          ...
          if (abbrevMonthNum) {
            return new Date(parseInt(match[3]), abbrevMonthNum - 1, parseInt(match[2]));
          }
          break;
