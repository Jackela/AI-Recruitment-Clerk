...
describe('ResumeRepository Integration', () => {
  ...
  describe('CRUD Operations', () => {
    ...
    it('should create a new resume', async () => {
      ...
      expect(createdResume.contactInfo.name).toBe(testResumeData.contactInfo.name);
      expect(createdResume.contactInfo.email).toBe(testResumeData.contactInfo.email);
      expect(createdResume.skills).toEqual(testResumeData.skills);
      expect(createdResume.status).toBe(testResumeData.status);
      
      createdResumeId = createdResume._id.toString();
    });
    ...
    it('should find resume by ID', async () => {
      ...
      expect(foundResume?._id.toString()).toBe(createdResumeId);
      expect(foundResume?.contactInfo.name).toBe(testResumeData.contactInfo.name);
    });

    it('should find resume by email', async () => {
      const resumesByEmail = await repository.findByEmail(testResumeData.contactInfo.email);
      
      expect(resumesByEmail).toBeDefined();
      expect(resumesByEmail.length).toBeGreaterThan(0);
      expect(resumesByEmail[0].contactInfo.email).toBe(testResumeData.contactInfo.email);
    });

    it('should find resume by GridFS URL', async () => {
      const resumeByUrl = await repository.findByGridFsUrl(testResumeData.gridFsUrl);
    ...
    it('should update resume by ID', async () => {
      ...
      const updatedResume = await repository.updateById(createdResumeId, updateData);
      
      expect(updatedResume).toBeDefined();
      expect(updatedResume?.processingConfidence).toBe(0.98);
      expect(updatedResume?.status).toBe('validated');
    });

    it('should update resume status', async () => {
      const updatedResume = await repository.updateStatus(createdResumeId, 'failed', 'Test error');
    ...
    it('should find resumes by status', async () => {
      ...
      expect(failedResumes.some(r => r._id.toString() === createdResumeId)).toBe(true);
    });
    ...
    it('should find resumes with specific skills', async () => {
      ...
      const resumesWithSkills = await repository.findWithSkills(['Python', 'JavaScript']);
      
      expect(resumesWithSkills).toBeDefined();
      expect(resumesWithSkills.some(r => 
        r.skills.includes('Python') && r.skills.includes('JavaScript')
      )).toBe(true);
    });
