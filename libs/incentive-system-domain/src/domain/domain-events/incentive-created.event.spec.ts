import { IncentiveCreatedEvent } from './incentive-created.event';
import { Currency, TriggerType } from '../aggregates/incentive.aggregate';

describe('IncentiveCreatedEvent', () => {
  const createEvent = (
    overrides?: Partial<{
      incentiveId: string;
      recipientIP: string;
    ...
    return new IncentiveCreatedEvent(
      ...
      overrides?.triggerType ?? TriggerType.QUESTIONNAIRE_COMPLETION,
  ...
  describe('constructor', () => {
    it('should create event with all properties', () => {
      ...
      expect(event.triggerType).toBe(TriggerType.REFERRAL);
    ...
    it('should create event with default values', () => {
      ...
      expect(event.triggerType).toBe(TriggerType.QUESTIONNAIRE_COMPLETION);
