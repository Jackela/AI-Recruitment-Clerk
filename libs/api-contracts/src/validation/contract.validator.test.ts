...
describe('ContractValidator', () => {
  describe('validateJobContract', () => {
    ...
    it('should detect missing required fields', () => {
      ...
      const result = ContractValidator.validateJobContract(invalidJob, 'JobBase');
      
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('id must be a non-empty string');
      expect(result.errors).toContain('status must be one of: draft, active, processing, completed, closed');
      expect(result.errors).toContain('createdAt must be a Date object');
      expect(result.errors).toContain('resumeCount must be a non-negative number');
    });
    ...
    it('should validate JobDetail requires jdText', () => {
      ...
      const result = ContractValidator.validateJobContract(jobWithoutJdText, 'JobDetail');
      
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('jdText must be a non-empty string for JobDetail');
    });
  ...
  describe('validateReportContract', () => {
    it('should validate valid analysis report', () => {
      ...
      const result = ContractValidator.validateReportContract(validReport, 'AnalysisReport');
    ...
    it('should detect invalid match score', () => {
      ...
      const result = ContractValidator.validateReportContract(invalidReport, 'ReportListItem');
      
      expect(result.isValid).toBe(false);
      expect(result.errors).toContain('matchScore must be a number between 0 and 100');
    });
    ...
    it('should validate AnalysisReport requires additional fields', () => {
      ...
      const result = ContractValidator.validateReportContract(incompleteReport, 'AnalysisReport');
  ...
  describe('compareContracts', () => {
    it('should detect matching contracts', () => {
      ...
      const result = ContractValidator.compareContracts(frontend, backend, 'TestContract');
  ...
  describe('integration tests', () => {
    it('should validate realistic frontend-backend job contract comparison', () => {
      ...
      const frontendValidation = ContractValidator.validateJobContract(frontendJob, 'JobDetail');
      const backendValidation = ContractValidator.validateJobContract(backendJobDto, 'JobDetail');
      
      expect(frontendValidation.isValid).toBe(true);
      expect(backendValidation.isValid).toBe(true);

      // Compare contracts
      const comparison = ContractValidator.compareContracts(frontendJob, backendJobDto, 'JobContract');
    ...
    it('should detect when frontend and backend job status enums diverge', () => {
      ...
      const result = ContractValidator.validateJobContract(frontendJob, 'JobBase');
