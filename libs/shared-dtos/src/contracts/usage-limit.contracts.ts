...
export class UsageLimitContracts {
   ...
   */
  static checkUsageLimit(ip: string, usageLimit: UsageLimit): UsageLimitCheckResult {
    // 前置条件验证
  ...
  static addBonusQuota(
    ...
    if (amount > UsageLimitRules.MAX_BONUS_QUOTA) {
      throw new Error(`Bonus amount cannot exceed maximum limit: ${UsageLimitRules.MAX_BONUS_QUOTA}`);
    }
