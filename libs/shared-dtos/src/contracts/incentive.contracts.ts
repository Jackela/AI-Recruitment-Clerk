...
export class IncentiveContracts {
  ...
  static createQuestionnaireIncentive(
    ...
    this.require(
      typeof qualityScore === 'number' && qualityScore >= 0 && qualityScore <= 100,
    ...
    // 执行创建
    const incentive = Incentive.createQuestionnaireIncentive(ip, questionnaireId, qualityScore, contactInfo);
  ...
  static validateInvariants(incentive: Incentive): void {
    ...
    if (status === IncentiveStatus.REJECTED || status === IncentiveStatus.EXPIRED) {
      // 拒绝或过期的激励不应该有支付信息
      // 这里可以添加更多特定状态的不变式
    }
  ...
  static validatePaymentCompatibility(
    ...
    const validation = IncentiveRules.validatePaymentMethodCompatibility(paymentMethod, contactInfo);
    this.require(
      validation.isValid,
