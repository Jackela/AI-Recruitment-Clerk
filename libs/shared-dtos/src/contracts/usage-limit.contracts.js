...
exports.UsageLimitContracts = void 0;
const usage_limit_dto_1 = require("../domains/usage-limit.dto");
const usage_limit_rules_1 = require("../domains/usage-limit.rules");
class UsageLimitContracts {
    /**
     * 合约方法 - 检查IP使用限制
     * 前置条件: IP地址格式有效
     * 后置条件: 返回结果包含正确的配额信息和状态
     */
    static checkUsageLimit(ip, usageLimit) {
        // 前置条件验证
     ...
     */
    static recordUsage(ip, usageLimit) {
        // 前置条件验证
        if (!usage_limit_rules_1.UsageLimitRules.isValidIPAddress(ip)) {
            throw new Error('IP address must be valid IPv4 format');
        }
        ...
        if (result.isSuccess()) {
            ...
            if (currentUsage === undefined || remainingQuota === undefined) {
                throw new Error('Successful result must have current usage and remaining quota');
            }
            // 验证使用计数递增
            if (currentUsage !== beforeUsage + 1) {
                throw new Error(`Usage count should increment by 1: expected ${beforeUsage + 1}, got ${currentUsage}`);
            }
            // 验证剩余配额递减
            if (remainingQuota !== beforeAvailable - 1) {
                throw new Error(`Remaining quota should decrement by 1: expected ${beforeAvailable - 1}, got ${remainingQuota}`);
            }
            // 验证配额一致性
            if (currentUsage + remainingQuota !== usageLimit.getUsageStatistics().availableQuota) {
                throw new Error('Current usage + remaining quota must equal total available quota');
            }
        }
        else {
     ...
     * 后置条件: 配额正确增加，事件正确发布
     */
    static addBonusQuota(ip, usageLimit, bonusType, amount) {
        // 前置条件验证
        if (!usage_limit_rules_1.UsageLimitRules.isValidIPAddress(ip)) {
            throw new Error('IP address must be valid IPv4 format');
        }
        ...
        if (amount > usage_limit_rules_1.UsageLimitRules.MAX_BONUS_QUOTA) {
            throw new Error(`Bonus amount cannot exceed maximum limit: ${usage_limit_rules_1.UsageLimitRules.MAX_BONUS_QUOTA}`);
        }
        // 捕获执行前状态
        ...
        if (afterAvailable !== beforeAvailable + amount) {
            throw new Error(`Available quota should increase by ${amount}: expected ${beforeAvailable + amount}, got ${afterAvailable}`);
        }
        // 验证奖励配额增加
        if (afterBonus !== beforeBonus + amount) {
            throw new Error(`Bonus quota should increase by ${amount}: expected ${beforeBonus + amount}, got ${afterBonus}`);
        }
        // 验证事件发布
        if (afterEvents !== beforeEvents + 1) {
            throw new Error('Adding bonus quota should generate exactly one domain event');
        }
    }
    /**
     * 合约方法 - 创建使用限制
     ...
     * 后置条件: 创建的对象具有正确的初始状态
     */
    static createUsageLimit(ip, policy) {
        // 前置条件验证
        if (!usage_limit_rules_1.UsageLimitRules.isValidIPAddress(ip)) {
            throw new Error('IP address must be valid IPv4 format');
        }
        ...
        if (!usage_limit_rules_1.UsageLimitRules.isValidUsagePolicy(policy)) {
            throw new Error('Usage limit policy must be valid according to business rules');
        }
        // 执行创建逻辑
        const usageLimit = usage_limit_dto_1.UsageLimit.create(ip, policy);
        // 后置条件验证
        if (!usageLimit) {
        ...
        if (usageLimit.getAvailableQuota() !== policy.dailyLimit) {
            throw new Error(`New UsageLimit should have available quota equal to daily limit: expected ${policy.dailyLimit}, got ${usageLimit.getAvailableQuota()}`);
        }
        const events = usageLimit.getUncommittedEvents();
        if (events.length !== 1) {
            throw new Error('Creating UsageLimit should generate exactly one creation event');
        }
     ...
     * 后置条件: 重置后状态正确
     */
    static validateDailyReset(usageLimit) {
        // 前置条件验证
        if (!usageLimit) {
            throw new Error('UsageLimit instance is required');
        }
        ...
        if (resetOccurred) {
            ...
            if (afterAvailable !== policy.dailyLimit) {
                throw new Error(`After daily reset, available quota should be daily limit: expected ${policy.dailyLimit}, got ${afterAvailable}`);
            }
            // 验证重置事件
            const events = usageLimit.getUncommittedEvents();
            const resetEvent = events.find(e => e.constructor.name === 'DailyUsageResetEvent');
            if (!resetEvent) {
     ...
     * 不变性验证 - 使用限制核心不变性
     */
    static validateInvariants(usageLimit) {
        if (!usageLimit) {
            throw new Error('UsageLimit instance is required for invariant validation');
        }
        const stats = usageLimit.getUsageStatistics();
        // 不变性1: 当前使用量不能超过可用配额
        if (stats.currentUsage > stats.availableQuota) {
            throw new Error(`Invariant violation: Current usage (${stats.currentUsage}) cannot exceed available quota (${stats.availableQuota})`);
        }
        // 不变性2: 当前使用量不能为负数
        if (stats.currentUsage < 0) {
            throw new Error(`Invariant violation: Current usage cannot be negative: ${stats.currentUsage}`);
        }
        // 不变性3: 可用配额必须包含基础配额
        if (stats.availableQuota < stats.dailyLimit) {
            throw new Error(`Invariant violation: Available quota (${stats.availableQuota}) cannot be less than daily limit (${stats.dailyLimit})`);
        }
        // 不变性4: 奖励配额不能为负数
        if (stats.bonusQuota < 0) {
            throw new Error(`Invariant violation: Bonus quota cannot be negative: ${stats.bonusQuota}`);
        }
        // 不变性5: IP地址格式必须有效
        if (!usage_limit_rules_1.UsageLimitRules.isValidIPAddress(stats.ip)) {
            throw new Error(`Invariant violation: IP address must be valid: ${stats.ip}`);
        }
        // 不变性6: 重置时间必须是未来时间
        const now = new Date();
        if (stats.resetAt <= now) {
            ...
            if (stats.resetAt.getTime() + allowedSkew <= now.getTime()) {
                throw new Error(`Invariant violation: Reset time must be in the future (with 1-minute tolerance): ${stats.resetAt}`);
            }
        }
    }
    /**
     * 性能合约 - 验证操作在可接受时间内完成
     */
    static performanceContract(operation, maxExecutionTimeMs = 100, operationName = 'Operation') {
        const startTime = Date.now();
        try {
            const result = operation();
            const executionTime = Date.now() - startTime;
            if (executionTime > maxExecutionTimeMs) {
                throw new Error(`Performance contract violation: ${operationName} took ${executionTime}ms, maximum allowed: ${maxExecutionTimeMs}ms`);
            }
            return result;
        }
        catch (error) {
            const executionTime = Date.now() - startTime;
            if (executionTime > maxExecutionTimeMs) {
                throw new Error(`Performance contract violation: ${operationName} took ${executionTime}ms (with error), maximum allowed: ${maxExecutionTimeMs}ms`);
            }
            throw error;
        }
    }
}
exports.UsageLimitContracts = UsageLimitContracts;
