  ...
  public static validateRequiredEnvironment(
    ...
    for (const varName of requiredVars) {
      ...
      if (value && this.isInsecureFallbackValue(value)) {
        errors.push(
          `Insecure fallback detected for ${varName}. Use proper environment variable.`,
        );
      }
    }
    ...
    if (isProduction) {
      ...
      for (const varName of criticalVars) {
        ...
        if (!value) {
          ...
          errors.push(
            `Production environment has insecure value for: ${varName}`,
          );
        }
      }
      ...
      if (errors.length > 0) {
        this.logger.error(
          '🚨 PRODUCTION SECURITY FAILURE - Service startup blocked',
        );
        errors.forEach((error) => this.logger.error(`   • ${error}`));
      }
    }
