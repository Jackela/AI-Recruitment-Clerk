...
describe('EncryptionService', () => {
  ...
  describe('Field-level Encryption', () => {
    it('should encrypt specific fields in an object', () => {
      ...
      const encrypted = EncryptionService.encryptFields(userData, ['firstName', 'lastName', 'email']);
    ...
    it('should decrypt specific fields in an object', () => {
      ...
      const encrypted = EncryptionService.encryptFields(userData, ['firstName', 'lastName', 'email']);
      const decrypted = EncryptionService.decryptFields(encrypted, ['firstName', 'lastName', 'email']);
    ...
    it('should handle complex object fields', () => {
      ...
      const encrypted = EncryptionService.encryptFields(complexData, ['metadata']);
      const decrypted = EncryptionService.decryptFields(encrypted, ['metadata']);
  ...
  describe('Security Validation', () => {
    ...
    it('should detect weak encryption keys', () => {
      ...
      expect(validation.errors).toContain('Encryption key must be at least 32 characters long.');
    });

    it('should detect default encryption key', () => {
      process.env.ENCRYPTION_MASTER_KEY = 'default-key-change-in-production-please-use-32-bytes-key';
      
      const validation = EncryptionService.validateConfig();
      expect(validation.isValid).toBe(false);
      expect(validation.errors).toContain('Using default encryption key. Please set ENCRYPTION_MASTER_KEY environment variable.');
    });
  ...
  describe('Error Handling', () => {
    ...
    it('should handle invalid encrypted data format', () => {
      expect(() => EncryptionService.decrypt(null as any)).toThrow('Cannot decrypt empty or invalid encrypted data');
      expect(() => EncryptionService.decrypt({} as any)).toThrow('Cannot decrypt empty or invalid encrypted data');
    });
    ...
    it('should handle decryption failures gracefully in field operations', () => {
      ...
      const decrypted = EncryptionService.decryptFields(userData, ['firstName']);
