...
export class StandardizedErrorResponseFormatter extends ErrorResponseFormatter {
  ...
  static formatEnhanced(
    ...
    const correlationContext = enhancedDetails.correlationContext || 
                              ErrorCorrelationManager.getContext();

    // Determine service name from correlation context or monitoring tags
    const serviceName = correlationContext?.serviceName || 
    ...
    const response: StandardizedErrorResponse = {
      ...
      recovery: {
        ...
        suggestions: this.getRecoverySuggestions(enhancedDetails.type, enhancedDetails.code),
    ...
    if (enhancedDetails.monitoringTags && Object.keys(enhancedDetails.monitoringTags).length > 0) {
      response.monitoring = {
  ...
  private static getUserFriendlyMessage(
    ...
    return super.formatUserMessage({ errorDetails: { type: errorType } } as any);
  }
  ...
  private static isRetryableError(error: EnhancedAppException): boolean {
    ...
    return !nonRetryableTypes.includes(error.enhancedDetails.type) &&
