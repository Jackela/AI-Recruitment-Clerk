...
export class UsageCacheService {
  ...
  async getTotalQuota(
    ...
    const questionnaireBonus = results?.[0]?.[1]
      ? parseInt(results[0][1] as string, 10)
      : 0;
    const paymentBonus = results?.[1]?.[1]
      ? parseInt(results[1][1] as string, 10)
      : 0;
  ...
  async getUsageStatus(
    ...
    const questionnaireBonus = results?.[1]?.[1]
      ? parseInt(results[1][1] as string, 10)
      : 0;
    const paymentBonus = results?.[2]?.[1]
    ...
    const resetTime =
      ttl > 0 ? new Date(Date.now() + ttl * 1000) : this.getNextMidnight();
