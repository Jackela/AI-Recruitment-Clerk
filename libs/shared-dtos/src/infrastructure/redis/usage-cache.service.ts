...
export class UsageCacheService {
   ...
   * 获取IP奖励配额
   */
  async getBonusQuota(ip: string, bonusType: 'questionnaire' | 'payment'): Promise<number> {
   ...
   * 增加奖励配额
   */
  async addBonusQuota(ip: string, bonusType: 'questionnaire' | 'payment', amount: number): Promise<number> {
    const key = this.getBonusKey(ip, bonusType);
  ...
  async getTotalQuota(ip: string, baseQuota = 5): Promise<{
    ...
    const questionnaireBonus = results?.[0]?.[1] ? parseInt(results[0][1] as string, 10) : 0;
    const paymentBonus = results?.[1]?.[1] ? parseInt(results[1][1] as string, 10) : 0;
  ...
  async getUsageStatus(ip: string, baseQuota = 5): Promise<{
    ...
    const used = results?.[0]?.[1] ? parseInt(results[0][1] as string, 10) : 0;
    const questionnaireBonus = results?.[1]?.[1] ? parseInt(results[1][1] as string, 10) : 0;
    const paymentBonus = results?.[2]?.[1] ? parseInt(results[2][1] as string, 10) : 0;
  ...
  private getDailyUsageKey(ip: string): string {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
    return `${this.USAGE_PREFIX}${this.DAILY_PREFIX}${today}:${ip}`;
  }

  private getBonusKey(ip: string, bonusType: 'questionnaire' | 'payment'): string {
    const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD
